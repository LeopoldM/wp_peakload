...
\begin{equation*}
    U(q,\theta,s) = \int_0^{q} u(\tilde{q},\theta,s) d\tilde{q}
\end{equation*}
...
\begin{equation*}
CS(q,t,\theta,s) = U(q,\theta,s) - t 
\end{equation*}

If a consumer does not receive electricity, we assume its value is null. Finally, we assume that $u$ is linear of the form: 
...
In the spirit of \cite{Wilson1989b,Wilson1989a}, we derive different mechanisms to allocate electricity. We notably differentiate between a \textit{general mechanism} and some \textit{market mechanisms}. In the former, the market designer independently selects the quantity and monetary transfer. It will serve as our benchmark allocation. In the latter, consumers choose their quantity schedule given a transfer schedule offered by the market designer. We formally define those below.

\noindent \textbf{General Mechanism}. Given a total quantity $Q(s)$ of electricity in state of the world $s$, a general allocation mechanism $\mathcal{M}$ can be described via a collection of functions $q_i: [\ubar{\theta}_i, \bar{\theta}_i] \rightarrow \Delta(Q(s)) $ where $q_i$ is a function describing the quantity $q$ of electricity allocation to a consumer with type $\theta$ in category $i$ at a state $s$. The aggregate quantity allocated to a group  $i$ of consumers is 

\begin{equation*}
    Q_i(s) = \mu_i \int_{\theta_i} q_i(\theta,s)  dG_i(\theta)
\end{equation*}

The total allocation is $Q(s) = \sum_{i = \{1,2\}} Q_i(s) = \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} q_i(\theta,s)  dG_i(\theta)$. We also define the function $t_i(\theta,s)$ as the monetary transfer assigned to a consumer with type $\theta \in[\ubar{\theta}_i, \bar{\theta}_i]$ in category $i$ at state $s$. To study the optimal second-best mechanism with incomplete information in section \ref{chap3sec:sec5}, we rely on the Revelation Principle. Given a direct mechanism $(q_i,t_i)_{i = \{1,2\}}$, for each category, consumers report their type $\theta$, receive an allocation $q_i(\theta)$, and pays $t_i(\theta)$ to the market designer. 

\noindent \textbf{Market mechanism}. In this paper, we also provide another pricing mechanism that we name \textit{market allocation}, which is  formally defined as follows. Given a transfer schedule $t(q,\theta,s)$, a consumer chooses a quantity schedule $d(p,\theta,s)$ such that: 

\begin{equation*}
d(p,\theta,s) = \max_q \{ U(q,\theta,s) - P(q,\theta,s) \} 
\end{equation*}

Therefore, a market allocation is an allocation in which a consumer chooses, optimally, its quantity allocation. We use the notation $d(p,\theta,s)$ to avoid any confusion with the general mechanism allocation $q(\theta,s)$. The former can be understood as an individual demand function. The comparison between the mechanism design and the market allocation will become handy for different reasons. First, market allocations are the primary mechanism used to allocate electricity. It notably encompasses different pricing mechanisms, such as spot-pricing where $t(q,\theta,s)$ is given by a market clearing condition and two-part tariffs where $t(q,\theta,s)$ is comprised of unit price and a lump-sum transfer. Second, having the general mechanism as a benchmark allows for comparing various forms of market allocations, notably in terms of consequences for consumer surplus. 

% we will use the fact that the market allocation is an \textit{implementable mechanism} mimicking a mechanism design outcome, or on the contrary, as a constraint for the market designer. To present the current pricing mechanisms, we use market allocation as a source of inefficiencies.


% The inverse demand functions for each category as $p_i(Q_i,s) = D^{-1}_i(Q_i,s)$, and the aggregate function for all consumers is given by $p(Q(s),s) = \sum_{i = \{1,2\}} D^{-1}_i(Q_i,s)$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{align*}
\max_{\substack{ t_i(\theta,s) , \\ q_i(\theta,s) }}  \quad \quad &  CS(k) =  \sum_{i = \{1,2\}}  \mu_i \int_s  \int_{\theta_i} \left(U(q_i(\theta,s),\theta,s)  -  t_i(\theta,s)\right) dG_i(\theta) dF(s) \quad  \\
  \text{s.t.} \quad\quad & I(k)   \leq   \sum_{i = \{1,2\}} \mu_i \int_s   \int_{\theta_i}  t_i(\theta,s)    dG_i(\theta)   dF(s) ,         \tag{R}\\
    &\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} q_i(\theta,s) dG_i(\theta) \leq k, \tag{K}
\end{align*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The allocation mechanism is \textit{feasible} if the two conditions are satisfied. The first condition is the budget constraint. It follows the principle that the market designer should avoid any negative revenue at the optimum level of investment. In other words, under the supply-side assumption and given the absence of production cost, the entire income is allocated to financing the investment costs. The second condition is the capacity constraint. We denote $\lambda$ its Lagrange multiplier. We also implicitly include conditions such that $q_i$ and $t_i$ are positive and that every consumer derives a null or positive surplus when participating in the mechanism.

We show in Proposition \ref{propch3:prop1} that under a general mechanism, the optimal allocation is defined by a unique quantity schedule. We define $s^* \equiv s^*(\textbf{q},k)$ as the first state of the world when the capacity is binding, given the set of quantity schedule $\textbf{q} = \{q_i(\theta,s)\}_{i = \{1,2\}}$:

\begin{equation*}
\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} q_i(\theta,s^*) dG_i(\theta) = k
\end{equation*}

We note $S^* = [\ubar{s},s^*]$ as the set of off-peak periods, during which the capacity is not biding, and $T^* = (s^*,\bar{s}]$ as the set of on-peak periods, during which the capacity is binding.  

\begin{proposition}\label{propch3:prop1}
The optimal allocation is defined for each realization of $s$ by the following quantity schedule satisfying the following conditions:

\begin{equation*}    
 q_i^*(\theta,s) = \quad \begin{cases}
    \quad \ u(q_i^*(\theta,s),\theta,s) = 0 \quad \text{if} \quad s \in S^* \\
   \quad \ u(q_i^*(\theta,s),\theta,s) = \lambda \quad \text{if} \quad s \in T^*
   \end{cases}
\end{equation*}
...
% %%%%%%%
% \begin{equation*}
% k^* = \left\{ \quad k \quad | \quad r = \sum_{i = \{1,2\}} \mu_i \int_{s_1}^{\bar{s}}\int_{\theta_i} u(k,\theta,s) dG_i(\theta) dF(s) \right\}
% \end{equation*}
...
% \begin{equation*}
%     p(q,s) = \sum_{i = \{1,2\}}\mu_i \int_s \int_{\theta_i} u(q,\theta,s) dG_i(\theta)dF(s)
% \end{equation*}
...
Solving the program shows that when the capacity is not biding, the optimal allocation is characterized by an expected marginal utility equal to the marginal cost. On the other hand, when the capacity is binding, the optimal allocation should be equal to the Lagrange multiplier attached to the capacity constraint. It implies that the optimal allocation is such that the marginal utility should be equal in every state of the world.

Due to the general form of the mechanism, the optimal allocation remains agnostic on the payment schedule. Under complete information, it is straightforward that if the welfare comprised of the expected utility net of investment cost is strictly positive at the optimal quantity schedule, then it is always feasible to design individual lump-sum transfers to allow for full cost recovery while ensuring a positive utility to every consumer. Discussions in the literature usually revolve around assuming \textit{a market allocation}, in which consumers choose their quantity schedule given some monetary transfers schedule. As explained in the previous section, spot pricing relies on a market-clearing condition. From Equation XXXX, we derive the aggregate demand function for a given realization of $s$ and a unit price price $p$: 

\begin{equation*}
    D(p,s) = \sum_{i = \{1,2\}} \mu_i \int_{\theta} d(p,\theta,s) dG_i(\theta)
\end{equation*}

The supply function is given by the marginal cost normalized to $0$, and by the capacity constraint $k$. Therefore, the market clearing condition implies that the price schedule to ensure demand equal supply should be such that when $D(0,s) \leq k $, then the price is at the marginal cost. When the capacity starts to bind, then there is a price noted $p(k,s)$ such that $D(p(k,s),s)  = k$. Graphically, it corresponds to the intersection between the aggregate inverse demand and the level of capacity $k$. The spot price schedule is therefore $\{0,p(k,s)\}$. We summarize the main result in the following lemma.

\begin{lemma}

$i)$ A spot pricing implements the first-best allocation only if the investment level is at the optimal level. $ii)$ Without any distortion, spot pricing also leads to the optimal investment level. $iii)$ With complete information, it is always possible to implement a two-part tariff that leads to a feasible allocation.
\end{lemma}

The results of this lemma are at the core of peak load pricing theory. Whenever the capacity is not constraining, efficiency implies that prices equal the short-term marginal cost, i.e., the marginal production cost, which is null in this framework. When the capacity is binding, the price deviates from efficiency and acts as a rationing mechanism that satisfies the capacity constraint. Given Equation XXXX, we have $u(q,\theta,s) = p$, which ensures the first-best allocation. However, this optimal allocation does not guarantee full cost recovery for the market designer. If the investment level is not at the value that maximizes welfare, then the budget constraint is not binding. This is particularly true for any investment level above the optimal level, which implies a negative budget. To see this, we express below the expected welfare under spot pricing:

\begin{equation*}
    W(k) = \sum_{i=\{1,2\}} \mu_i\{ \int_{S^*} \int_{\theta_i} U(d(0,\theta,s)) dG_i(\theta) dF(s) +  \int_{T^*} \int_{\theta_i} U(d(p(k,s),\theta,s)) dG_i(\theta) dF(s) \} - I(k)
\end{equation*}

The first-order condition with respect to $k$ implies that 

\begin{equation*}
    \int_{T^*} \int_{\theta_i} p(k,s) dG_i(\theta) dF(s) = I'(k) 
\end{equation*}

This is equivalent to the maximization of the program XXXX with respect to $k$, which implies that $\lambda = I'(K)$. Under spot pricing, the optimal investment level is given by the equality between the marginal expected surplus during on-peak periods (i.e., $T^*$) and the marginal investment cost. It also ensures complete coverage of the investment cost and the expected revenue. Given the market clearing condition, it is straightforward that if $k$ deviates from the optimal level, this will not be ensured. The last part of the Lemma relies on the previous remark that under complete information, it is always possible to find a set of lump sum transfers that covers investment costs. Then, it is always possible to implement a \textit{feasible} two-part tariff with a unit price based on spot pricing completed with individualized lump sum transfers to cover investment costs.\footnote{In the context of heterogeneous consumers, this assumption of differentiated transfers is crucial. See, for instance, \cite{Tirole1993} for a discussion of the consequences of a uniform fixed fee in terms of participation in the mechanism.}
...
% \begin{equation*}
%     \sum_{i = \{1,2\}} \mu_i  \left[\int_0^{s_1}\ \underbrace{ \int_{\theta_i} U(d(0,\theta,s),\theta,s) dG_i(\theta)}_\text{off-peak utility} dF(s)        +  \int_{s_1}^{\bar{s}} \underbrace{\int_{\theta_i} U(d( p^k,\theta,s),\theta,s) dG_i(\theta)}_\text{on-peak utility}dF(s) \right]
% \end{equation*}
...
We start by defining the rationing policy under this framework. This stage boils down to allocating the capacity $k$ in the first step between the two categories and the second step, randomly for each consumer within each category.
...
\begin{equation*}
    \alpha_i \; k = \int_{\theta_i} q^*_i(\theta,s) dG_i(\theta)
\end{equation*}
...
% Let the quantity for category $i$ be $q_i$; then, when the capacity is constraining, we must have for every state of the world: $\sum_{i = \{1,2\}} \mu_i q_i  = k$, implying that the relation between the quantity is equal to $q_i(q_j) = \frac{k - \mu_{j}q_j}{\mu_i}$. Then, we maximize the short-term expected utility given the previous relation. Solving using the first-order condition leads to a capacity share for each consumer belonging to a group $i$ equals the allocation under the first-best allocation. 


\subsection{Optimal single prices}


Given the rationing policy, we now describe the new problem the market designer faces. We define $s^r \equiv s^r(\textbf{t}^r,k)$ as the first state of the world when the capacity is binding, given the set of fixed price $\textbf{t}^r = \{t^r_i\}_{i = \{1,2\}}$:

\begin{equation*}
\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} d(t^r_i,\theta,s^r) dG_i(\theta) = k
\end{equation*}

We note $S^r = [\ubar{s},s^r]$ as the set of off-peak periods during which the capacity is not biding and $T^r = (s^r,\bar{s}]$ as the set of on-peak periods during which the capacity is binding. Then, we define the expected aggregate consumer surplus in each set. This is key as prices do not affect consumers in the same way, depending on the period. The off-peak surplus is given by:

\begin{equation*}
    CS_S^r(k,\textbf{t}^r) = \sum_{i = \{1,2\} } \mu_i \int_{S^r} \int_{\theta_i} \left( U(d(t^r_i,\theta,s),\theta,s) - t^r_i \; d(t^r_i,\theta,s) \right) dG_i(\theta) dF(\theta) 
\end{equation*}

and the on-peak surplus by:

\begin{equation*}
    CS_T^r(k,\textbf{t}^r) = \sum_{i = \{1,2\} } \mu_i \int_{T^r} \int_{\theta_i} ( U\left(\frac{\alpha_i k}{\mu_i},\theta,s\right) - t^r_i \; \frac{\alpha_i k}{\mu_i} ) dG_i(\theta) dF(\theta)
\end{equation*}
...
\begin{equation*}
    R_S^r(k,\textbf{t}^r) = \sum_{i = \{1,2\} } \mu_i \int_{S^r} \int_{\theta_i} t^r_i \; d(t^r_i,\theta,s)  dG_i(\theta) dF(\theta) 
\end{equation*}

and the on-peak surplus by:

\begin{equation*}
    R_T^r(k,\textbf{t}^r) = \sum_{i = \{1,2\} } \mu_i \int_{T^r} \int_{\theta_i} t^r_i \; \frac{\alpha_i k}{\mu_i} dG_i(\theta) dF(\theta)
\end{equation*}
...
% \begin{equation*}
%    \sum_{i = \{1,2\}}\mu_i \int_{\theta_i} d(t^r_i,\theta,s^r_1) dG_i(\theta) = k  
% \end{equation*}
...
% \begin{equation*}
%     \gamma^r_k =   -\frac{1}{bH^r}\left(\sum_{i = \{1,2\}} R_i ( L_{ij} L_{jk} - L_{jj} L_{ik} ) + R_k H^r \right)
% \end{equation*}
...
%     \pdv{t^r_i}{k} =&  \overbrace{\Bigg. \frac{-\mathcal{L}_{jj}}{H^r} \Bigg. }^{\leq 0} \overbrace{\Bigg( CS_{ik} +  \rho_i CS_{jk} +   (R_i - \rho_i R_j) \sum_{i = \{1,2\}}( CS_{ik} -\rho_i  CS_{jk} )\frac{ {  R_i } }{L_{jj} 
%  bH^r}}^{\text{effect of $k$ on CS with holding R fixed}}  \\ 
%    & \quad + \underbrace{  \gamma^r \bigg (R_{ik} - \rho_i R_{jk} + (R_i - \rho_i R_j)\sum_{i = \{1,2\}}(   R_{ik} - \rho_i R_{jk} ) \frac{ R_i  }{L_{jj} bH^r} \bigg)  -  R_k\frac{{ H^r} }{bH^r}   \Bigg) }_{\text{effect of $k$ on $R$}}
% \end{align}
...
% \begin{equation*}
%     \gamma^r_k =\frac{ \sum_{i = \{1,2\}}  R_i (L_{jj} L_{ik} - L_{ij} L_{jk}) - R_k H^r }{bH^r}
% \end{equation*}
...
% \begin{equation*}
%     \gamma^r_k =\frac{ \overbrace{\sum_{i = \{1,2\}}  R_i (  L_{jj} CS_{ik} - L_{ij} CS_{jk} )}^{\text{CS effect}} + \overbrace{\sum_{i = \{1,2\}} \gamma^r R_i (  L_{jj} R_{ik} - L_{ij} R_{jk} ) - R_k H^r}^{\text{Revenue effect}} }{bH^r}
% \end{equation*}
...
% \begin{align*}
%     \pdv{t^r_i}{k} =&  \overbrace{\Bigg. \frac{-\mathcal{L}_{jj}}{H^r} \Bigg. }^{\leq 0} \Bigg( \overbrace{CS_{ik} +  \rho_i CS_{jk} +   (R_i - \rho_i R_j) \sum_{i = \{1,2\}}( CS_{ik} -\rho_i  CS_{jk} )\frac{ {  R_i } }{L_{jj} 
%  bH^r}}^{\text{effect of $k$ on CS with holding R fixed}}  \\ 
%    & \quad + \underbrace{  \gamma^r \bigg (R_{ik} - \rho_i R_{jk} + (R_i - \rho_i R_j)\sum_{i = \{1,2\}}(   R_{ik} - \rho_i R_{jk} ) \frac{ R_i  }{L_{jj} bH^r} \bigg)  -  R_k\frac{{ H^r} }{bH^r} }_{\text{effect of $k$ on $R$}}  \Bigg) 
% \end{align*}
...
% \begin{align*}
%          &  \overbrace{ \Bigg. \sum_{i = \{1,2\}}\mu_i \int_{\theta_i} t^r_i \bigg( d(t^r_i,\theta,s^r_1) - \alpha_i^r k \bigg) dG_i(\theta)  \Bigg.}^{\text{$\Delta$ in quantity btw. off-peak and on-peak}}+ \overbrace{\int_{s^r_1}^{\bar{s}}  \Bigg. \sum_{i = \{1,2\}}\mu_i t^r_i dF(s)\Bigg. }^{\text{$+$ in on-peak rev.}}   - r
% \end{align*}
...
\max_{\substack{ k  }} \max_{\substack{ t_i(\theta,s) , \\ q_i(\theta,s) }}  \quad \quad &  CS(k) =  \sum_{i = \{1,2\}}  \mu_i \int_s     \int_{\theta_i} (U(q_i(\theta,s),\theta,s)    -  t_i(\theta,s) q_i(\theta,s)) dG_i(\theta) dF(s) \quad  \\
  \text{s.t.} \quad\quad & I(k) \leq \sum_{i = \{1,2\}} \mu_i \int_s  \int_{\theta_i}( t_i(\theta,s) q_i(\theta,s)) dG_i(\theta) dF(s) ,         \tag{R}\\
    & \sum_{i = \{1,2\}} \mu_i \int_s \int_{\theta_i} q_i(\theta,s) dG_i(\theta) dF(s) \leq k, \tag{K} \\
...
\begin{equation}\label{eq:RIRconstr}
        \sum_{i = \{1,2\}} \mu_i \int_s \int_\theta (U(q_i(\theta,s),\theta,s)  -\Gamma_i(\theta) \int_s q_i(\theta,s) dF(s) )dG_i(\theta) dF(s) - I(k) \geq 0 , \tag{R-IR}
\end{equation}
...
\begin{equation*}
    \mathbb{E}J_{4} = \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} J_i(q^m_{i,4},\theta,s) dG_i(\theta)
\end{equation*}
...
\end{proof}
 ...
 \begin{equation*}
     J_i(q^m_{i,4},\theta,s)  - \mathbb{E}J_{4} =( \theta - \Bar{\theta}_i + \sum_{i = \{1,2\}} \mu_i (\bar{\theta}_i - \ubar{\theta}_i))\frac{1}{1+\zeta(k)}
 \end{equation*}. 
...
% \begin{align*}
%     \sum_{i = \{1,2\}} \mu_i  \overbrace{ \left [\int_{0}^{s_1^m} \int_{\theta_i} \int_{\ubar{\theta}_i}^{\theta}  \pdv{q_{i,3}^m}{k} d\hat{\theta} dG_i(\theta) dF(s) \right. }^{\text{off-peak marginal loss}}  +  \overbrace{ \left.\int_{s_1^m}^{\Bar{s}} \int_{\ubar{\theta}_i}^{\hat{\theta}_i} \int_{\ubar{\theta}_i}^{\theta}    \pdv{q_{i,4}^m}{k} d\hat{\theta} dG_i(\theta) dF(s)  \right ]}^{\text{on-peak marginal loss}}  \\ 
%     = \underbrace{\sum_{i = \{1,2\}} \mu_i  \int_{s_1^m}^{\Bar{s}} \int_{\hat{\theta}_i}^{\bar{\theta}_i} \int_{\ubar{\theta}_i}^{\theta}    \pdv{q_{i,4}^m}{k} d\hat{\theta} dG_i(\theta) dF(s) }_{\text{on-peak marginal gain}}
% \end{align*}
...
\begin{equation*}
    R^r_0(k,t^r):= t^r  \quad \underbrace{\left(\sum_{i = \{1,2\}}\mu_i \int_{S^r_0} \int_{\theta_i} d(t^r,\theta,s) dG_i(\theta) dF(s) + \int_{T^r_0}  k dF(s) \right)}_{\text{Expected quantity}} \quad -  \quad  I(k) 
\end{equation*}
...
Let's define $\theta^r \equiv \theta^r(p,A)$ such 

\begin{equation*}
    \int_S (U(d(t^r,\theta^r,s) - A - t^r d(t^r,\theta^r,s) ) dF(s) = 0  
\end{equation*}

Note that the threshold is the same for the two categories of consumers. The set of consumers participating in the market from the category $i$ is given by $\boldsymbol{\uptheta}_i = (\theta_i^r , \bar{\theta}_i)$. The expected aggregate quantity is then: 

\begin{equation*}
    \mathbb{E}{Q_{\boldsymbol{\uptheta}}}(t^r) = \sum_{i = \{1,2\} } \mu_i \int_S  \int_{\boldsymbol{\uptheta}_i}  d(t^r,\theta,s) dG_i(\theta) dF(s)
\end{equation*}

The average quantity:

\begin{equation*}
    \mathbb{E}\tilde{Q_{\boldsymbol{\uptheta}}}(t^r) = \frac{ \mathbb{E}{Q}(t^r)}{\sum_{i = \{1,2\} } \mu_i \int_S  \int_{\boldsymbol{\uptheta}_i}g_i(\theta) d\theta}
\end{equation*}

The expected quantity demanded by the indifferent consumer in both groups:

\begin{equation*}
    \mathbb{E}d(t^r,\theta^r) =  \int_S  d(t^r,\theta^r,s)  dF(s)
\end{equation*}

And the expected derivative:

\begin{equation*}
    \mathbb{E}Q_{\boldsymbol{\uptheta}}'(t^r) = \sum_{i = \{1,2\} } \mu_i \int_S  \int_{\theta_i}  d'(t^r,\theta,s) dG_i(\theta) dF(s)
\end{equation*}


We denote the price elasticity of demand is $\varepsilon_p = \mathbb{E}Q_{\boldsymbol{\uptheta}}'(t^r) \frac{t^r}{\mathbb{E}Q_{\boldsymbol{\uptheta}}(t^r)}$. Then, the optimal two-part tariff satisfies the following condition.

\begin{equation*}
    1 = \frac{1}{1 + \lambda} \frac{1}{\varepsilon_p} \left[ 1 - \frac{\mathbb{E}d(t^r,\theta^r)}{\mathbb{E}\tilde{Q_{\boldsymbol{\uptheta}}}(t^r)}  \right]
\end{equation*}

With only a single price, the optimal $t^r$ is the average cost given by:

\begin{equation*}
    t^r = \frac{I(k)}{ \mathbb{E}{Q}(t^r)}
\end{equation*}

With 
\begin{equation*}
    \mathbb{E}{Q}(t^r) = \sum_{i = \{1,2\} } \mu_i \int_S  \int_{\theta_i}  d(t^r,\theta,s) dG_i(\theta) dF(s)
\end{equation*}
...
\begin{align*}
\max_{\substack{ t_i(\theta,s) \rightarrow \mathbb{R}^+, \\ q_i(\theta,s) \rightarrow \mathbb{R}^+ , \\ k \geq 0 }}  \quad \quad &  CS(k) =  \sum_{i = \{1,2\}}  \mu_i \int_s  \int_{\theta_i}\left( U(q_i(\theta,s),\theta,s)    -  t_i(\theta,s) q_i(\theta,s) \right) dG_i(\theta) 
dF(s) \quad  \\
  \text{s.t.} \quad\quad & I(k)   \leq   \sum_{i = \{1,2\}} \mu_i \int_s   \int_{\theta_i} t_i(\theta,s) q_i(\theta,s) dG_i(\theta)
     dF(s) ,         \tag{R}\\
    &\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} q_i(\theta,s) dG_i(\theta) \leq k, \tag{K}
\end{align*}
...
\begin{align*}
\max_{\substack{ t_i(\theta,s) \rightarrow \mathbb{R}^+, \\ q_i(\theta,s) \rightarrow \mathbb{R}^+ , \\ k \geq 0 }}  \quad \quad &  U^*(k) =  \sum_{i = \{1,2\}} \mu_i \int_s \int_{\theta_i} U(q_i(\theta,s),\theta,s)  dG_i(\theta) dF(s)  - I(k)\quad  \\
  \text{s.t.}     &\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} q_i(\theta,s) dG_i(\theta) \leq k, \tag{K}
\end{align*}

The corresponding Lagrangian is equal to 

\begin{align*}
    \mathcal{L} = \sum_{i = \{1,2\}}  \mu_i \int_s     \int_{\theta_i} U(q_i(\theta,s),\theta,s)  dG_i(\theta) dF(s)   + \varepsilon \left(k - \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} q_i(\theta,s) dG_i(\theta) \right)
\end{align*}
...
\begin{align*}
    ...
    k - \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} q_i(\theta,s) dG_i(\theta) = 0
\end{align*}
...
\begin{align*}
   U^*(k) =  \sum_{i = \{1,2\}} \mu_i &  \left[ \int_{0}^{s_1} \int_{\theta_i} U(q^*_{i,1}(\theta,s),\theta,s)  dG_i(\theta) dF(s) +  \right. \\
...
\begin{align*}
   \pdv{U^*(k)}{k} & =  \sum_{i = \{1,2\}}  \mu_i \int_{s_1}^{\Bar{s}} \int_{\theta_i} u(q^*_{i,2}(\theta,k,s),\theta,s) \pdv{q^*_{i,2}(\theta,k,s)}{k}   dG_i(\theta) dF(s) - r \\
   & =  \sum_{i = \{1,2\}}  \mu_i \int_{s_1}^{\Bar{s}} \int_{\theta_i} u(q^*_{i,2}(\theta,k,s),\theta,s) dG_i(\theta) dF(s) - r
\end{align*}
...
\begin{align}\label{eq:CSri}
     CS^r(k,t^r_i,t_j^r)  = & \sum_{i = \{1,2\}} \mu_i \left[\int_0^{s_1^r(k)}  \int_{\theta_i} \left( U(d(t^r_i,\theta,s),\theta,s) -  t^r_i d(t^r_i,\theta,s) \right)dG_i(\theta) dF(s) \right. \\
     \nonumber + & \left.\sum_{i = \{1,2\}} \mu_i \int_{s_1^r(k)}^{\Bar{s}}  \int_{\theta_i} \left( U(\alpha_i^r(k) k ,\theta,s) -  t^r_i \alpha_i^r(k) k \right)dG_i(\theta) dF(s)   \right]
\end{align}
...
\begin{equation}\label{eq:s1pr1}
      s_1^r(k) = k + \sum_{i = \{1,2\}} \mu_i (t_i^r - \theta_i^{av}) 
\end{equation}

The expected revenue net of investment costs is defined as:

\begin{align}\label{eq:Rri}
    R^r(k,t_i^r,t_j^r)  = & \sum_{i = \{1,2\}} \mu_i \left[\int_0^{s_1^r(k)}  \int_{\theta_i} \left(  t^r_i d(t^r_i,\theta,s) \right)dG_i(\theta) dF(s) \right. \\
     + & \left.\sum_{i = \{1,2\}} \mu_i \int_{s_1^r(k)}^{\Bar{s}}  \int_{\theta_i} \left( t^r_i \alpha_i^r(k) k \right)dG_i(\theta) dF(s)   \right] - I(k)
\end{align}
...
\begin{equation*}
    \pdv{\gamma(k)}{k}  =   -\frac{1}{bH^r}\left(\sum_{i = \{1,2\}} R_i  ( \mathcal{L}_{ij}^r  \mathcal{L}_{jk}^r  - \mathcal{L}_{jj}^r  \mathcal{L}_{ik}^r  ) + R_k H^r \right)
\end{equation*}
...
\begin{equation*}
    2 (2\gamma(k) - 1)^2 \sum_{i = \{1,2\}} \mu_i^3 \mu_j^2 (2\bar{s} - 2t_i^r + \theta_i^{av})^2
\end{equation*}
...
With $\mathbb{G} = 4\sum_{i = \{1,2\}} \mu_i (\theta_i^{av})^2 + \bar{s}(\bar{s} + 4 \sum_{i = \{1,2\}} \mu_i \theta_i^{ab})$ which is positive implying that the difference is also positive.
...
\begin{align*}
    \mathbb{B} = & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}\left(   CS(d(t_i^r,\theta,s_1^r(k)),\theta,s_1^r(k)) - CS(\alpha_i^r(k) k,\theta,s_1^r(k)) \right)dG_i(\theta) f(s)
\end{align*}
...
\begin{align*}
   \mathbb{C} = & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}\left(   t^r_i d(t_i^r,\theta,s_1^r(k)) - t^r_i \alpha_i^r(k) k  \right)dG_i(\theta)  f(s)
\end{align*}
...
This expression is close to $\mathbb{B}$ in the sense that it represents the cost adjustment for consumers due to the discontinuity of the individual consumer surplus. Note that we have $\mathbb{B} = \mathbb{D} - \mathbb{C} $, with $\mathbb{D} 
 = \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}\left(   U(d(t_i^r,\theta,s_1^r(k)),\theta,s_1^r(k)) - U(\alpha_i^r(k) k,\theta,s_1^r(k)) \right)dG_i(\theta) f(s)$, which is the utility adjustment. Therefore, we rewrite the Lagrange multiplier such that $\lambda^r = -\frac{CS^r_i}{R^r_i}$. It implies that: $\frac{CS^r_i}{R^r_i} = \frac{CS^r_j}{R^r_j}$. After rewriting the different terms, the optimal prices need to satisfy the following conditions:
...
\begin{align}\label{eq:pdvCSk}
    CS^r_k(k,t_i^r,t_j^r)  =& \mathbb{B}  + \sum_{i = \{1,2\}} \mu_i \int_{s_1^r(k)}^{\Bar{s}}  \int_{\theta_i} ( u(\alpha_i^r(k) k ,\theta,s) - t_i^r)dG_i(\theta) dF(s)
\end{align}
...
\begin{align*}
    R^r(k,t_i^r,t_j^r)  = & \sum_{i = \{1,2\}} \mu_i \int_s  \int_{\theta_i} \left(  t^r_i d(t^r_i,\theta,s) \right)dG_i(\theta) dF(s) - I(k)
\end{align*}
...
\begin{equation*}
     \sum_{i = \{1,2\}} \mu_i \int_{0}^{s^r_0(k,t^r)}\int_{\theta_i} (t^r d(t^r,\theta,s))dG_i(\theta) dF(s)
\end{equation*}

 The expected on-peak revenue is equal to :

\begin{equation*}
     \sum_{i = \{1,2\}} \mu_i \int_{s^r_0(k,t^r)}^{\Bar{s}}\int_{\theta_i} (t^r k)dG_i(\theta) dF(s)
\end{equation*}

With $s^r_0(k,t^r)$ defined as the first state of the world such that the capacity is binding:

\begin{equation*}
   \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} d(t^r,\theta,s^r_0(k,t^r))dG_i(\theta) = k
\end{equation*}
...
\begin{align} \label{eq:Revtr}
    R^r_0(k,t^r) = \sum_{i = \{1,2\}} \mu_i \left[ \int_{0}^{s^r_0(k,t^r)}\int_{\theta_i} (t^r d(t^r,\theta,s))dG_i(\theta) dF(s) + \right.  \nonumber \\ 
...
\begin{equation} 
        \sum_{i = \{1,2\}} \mu_i \int_{0}^{\Bar{s}}\int_{\theta_i} (t^r k)dG_i(\theta) dF(s) = I(k) \quad  \Rightarrow \quad  t^r = \frac{I(k)}{k}
 \end{equation}

The optimal transfer is the average investment cost. Under our framework, it is equal to $t^r= r$ for a value of $k^-(r)$ such that $s_0^r(k^-(r)) = 0 $, which is $k^-(r) =\sum_{i = \{1,2\}} \mu_i \theta_i^{av} - r $. The other extreme case is given under our framework by the expression:

\begin{equation*}
     \sum_{i = \{1,2\}} \mu_i \int_{0}^{\Bar{s}}\int_{\theta_i} (t^r d(t^r,\theta,s))dG_i(\theta) dF(s) = I(k) \quad  \Rightarrow \quad  t^r \left(\Bar{s}\frac{1}{2} - t^r - \sum_{i = \{1,2\}} \mu_i \theta_i^{av} \right) = I(k)
\end{equation*}

The first derivative with respect to $t^r$ is given by:

\begin{equation} \label{eq:pdvtrRext}
     \pdv{R^r_0(k,t^r)}{t^r} = \Bar{s}\frac{1}{2} - 2t^r - \sum_{i = \{1,2\}} \mu_i \theta_i^{av} 
\end{equation}

And the maximum is given by $t^r = \frac{1}{2} \left (\Bar{s}\frac{1}{2} - \sum_{i = \{1,2\}} \mu_i \theta_i^{av}  \right) $. The second derivative is equal to $-2$, which is negative, confirming the expected revenue is concave in $t^r$. Note also that the expected revenue is independent of $k$. This implies that the revenue net of investment cost is always decreasing in $k$. Discarding the case of corner solutions and lower limit of $k$ such that $s_0^r(k,t^r) = \Bar{s}$, a solution to the problem exists if and only if the net revenue at the maximum is positive, that is only when $k < \frac{(\Bar{s}\frac{1}{2} + \sum_{i = \{1,2\}} \mu_i \theta_i^{av})^2}{4r}$. The interpretation of this limit is that beyond it, the investment cost is such that the price to cover it leads to a negative consumer surplus. That is, the reaction to the price from the consumer prevents the financing of such an  investment level. 
...
\begin{align*}
    R^r_0(k) = \sum_{i = \{1,2\}} \mu_i  \int_{0}^{s^r_0(k,t^r)}\int_{\theta_i} (t^r d(t^r,\theta,s))dG_i(\theta) dF(s) +     \left( \sum_{i = \{1,2\}} \mu_i \int_{s^r_0(k,t^r)}^{\Bar{s}} t^r dF(s)- r \right) k  
\end{align*}

Hence, the second term in parenthesis is precisely the derivative of the revenue with respect to $k$:

\begin{align*}
    R^r_0(k) = \sum_{i = \{1,2\}} \mu_i  \int_{0}^{s^r_0(k,t^r)}\int_{\theta_i} (t^r d(t^r,\theta,s))dG_i(\theta) dF(s) +  \pdv{R^r_0(k)}{k} k  
\end{align*}
...
\begin{equation*}
    \pdv{R^r_0(k)}{{t^r}} =  \sum_{i = \{1,2\}} \mu_i \left[ \int_{0}^{s^r_0(k,t^r)}\int_{\theta_i} \left(d(t^r,\theta,s)  + t^r \pdv{d(t^r,\theta)}{t^r} \right)dG_i(\theta) dF(s) +  \int_{s^r_0(k,t^r)}^{\Bar{s}} k dF(s) \right] 
\end{equation*}
...
\begin{equation*}
    \pdv{R^r_0(k^-(r))}{{t^r}} =  \sum_{i = \{1,2\}} \mu_i  \int_{0}^{\Bar{s}} k dF(s) > 0
\end{equation*}
...
\begin{align*}
  \pdv[2]{R^r_0(k)}{{t^r}} =  f(s_0^r(k,t^r)) \pdv{s_0^r(k,t^r)}{t^r}   \left( \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} \left(d(t^r,\theta,s_0^r(k,t^r))  + t^r \pdv{d(t^r,\theta)}{t^r} \right)dG_i(\theta) -  k \right) \\ 
    +   \sum_{i = \{1,2\}} \mu_i \left[ \int_{0}^{s^r_0(k,t^r)}\int_{\theta_i} \left(2 \pdv{d(t^r,\theta)}{t^r}  + t^r \pdv[2]{d(t^r,\theta)}{{t^r}} \right)dG_i(\theta) dF(s)  \right] 
\end{align*}

Under linear marginal utility, we have  $ \pdv[2]{d(t^r,\theta)}{{t^r}} = 0$, which ensure that the revenue during off-peak is concave in $t^r$. Next, note that, by definition, we have the equality   $  \sum_{i = \{1,2\}} \mu_i  \int_{\theta_i}d(t^r,\theta,s_0^r(k,t^r)) dG_i(\theta) =  k$. This allows us to express the following derivative:

\begin{align*}
      \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \left( \pdv{s_0^r(k,t^r)}{t^r} \pdv{d(t^r,\theta)}{s} + \pdv{d(t^r,\theta)}{t^r} \right)dG_i(\theta) = 0 
\end{align*}
...
Isolating $CS(\ubar{\theta}_i)$ gives : $CS(\ubar{\theta}_i) = CS(\theta)  - \int_s \int_{\ubar{\theta}_i}^{\theta} q(\Tilde{\theta},s) d\Tilde{\theta} dF(s)$. Note that it is by definition independent of any consumer type, hence: $CS(\ubar{\theta}_i) = \int_{\theta_i} CS(\ubar{\theta}_i) dG_i(\theta)= \int_{\theta_i}\left(CS(\theta) - \int_s \int_{\ubar{\theta}_i}^{\theta} q(\Tilde{\theta},s) d\Tilde{\theta} dF(s) \right)dG_i(\theta)$. The smallest type from both groups needs to satisfy the participation constraint, hence $\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} CS(\ubar{\theta}_i) dG_i(\theta) \geq 0$. Then, it remains to note that the net revenue also needs to be at least positive. Note $R^m$ the expected revenue is given by :  

\begin{equation}\label{eq:Rm}
    R^m = \sum_{i = \{1,2\}} \mu_i \int_s \int_{\theta_i} (t_i(\theta,s) q_i(\theta,s)) dG_i(\theta) dF(s) - I(k) \geq 0 
\end{equation}

Combining both conditions leads to the following: 

\begin{align*}\label{eq:Rm}
   \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}\left(CS(\theta) - \int_s \int_{\ubar{\theta}_i}^{\theta} q(\Tilde{\theta},s) d\Tilde{\theta} dF(s) \right)dG_i(\theta) +  R^m \geq 0 
\end{align*}
...
\begin{align*}
   \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}\int_s\left(U(q(\theta,s),\theta,s) - \int_{\ubar{\theta}_i}^{\theta} q(\Tilde{\theta},s) d\Tilde{\theta} dF(s) \right) dF(s)dG_i(\theta)  - I(k) \geq 0 
\end{align*}
...
\begin{align*}
    \mathcal{L}^m & = \sum_{i = \{1,2\}} \mu_i \int_s \int_{\theta_i} (U(q_i(\theta,s),\theta,s) - q_i(\theta,s)t_i(\theta,s)) dG_i(\theta)dF(s) \\
    + & \varepsilon \left( k - \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}q_i(\theta,s)dG_i(\theta) \right) +  \zeta \; R-IR
\end{align*}

With $\varepsilon$ being the Lagrange multiplier associated with the capacity constraint and $\zeta$ the multiplier associated with the new participation/revenue constraint. Now, note that as the market designer seeks to maximize consumer surplus, having a positive net revenue is not optimal. In other terms, at the maximum we have $\sum_{i = \{1,2\}} \mu_i \int_s\int_{\theta_i}q_i(\theta,s)t_i(\theta,s)dG_i(\theta) =I(k)$. From the expression of $R-IR$ in Equation \ref{eq:RIR}, the derivative with respect to $q_i$ gives:
...
\begin{align}\label{eq:RIR}
        R^{IR}(k) = \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} \left[ \int_{0}^{s_1}  (U(q^m_{i,1}(\theta,s),\theta,s)  -\Gamma_i(\theta)q^m_{i,1}(\theta,s) ) dF(s) + \right. \\
...
\begin{align*}
        R^{IR}(k) = \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \int_s  (U(q^m_{i,2}(\theta,s),\theta,s)  -\Gamma_i(\theta)q^m_{i,2}(\theta,s) ) dF(s) dG_i(\theta) - I(k) 
\end{align*}

With the linear and uniform assumption, we have $q^m_{i,2}(\theta,s)= k + s + \theta - \sum_{i = \{1,2\}} \mu_i(s + \theta_i^{av})$ and $\Gamma_i(\theta) = \bar{\theta}_i - \theta$. In that case: 

\begin{equation*}
R^{IR}(0) =  \frac{1}{8} ( (\sum_{i = \{1,2\}} \mu_i (\bar{\theta}_i-\ubar{\theta}_i))^2+4\mu_i\mu_j(\bar{\theta}_i - \bar{\theta}_j)^2)  
\end{equation*}

Which is always positive. Next, we compute the first derivative with respect to $k$, which gives:

\begin{align*}
        \pdv{R^{IR}}{k} = & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \int_s  (u(q^m_{i,2}(\theta,s),\theta,s)\pdv{q^m_{i,2}}{k}  -\Gamma_i(\theta)\pdv{q^m_{i,2}}{k}(\theta,s) )dF(s) dG_i(\theta)- r \\
        & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \int_s  (u(q^m_{i,2}(\theta,s),\theta,s) -\Gamma_i(\theta)(\theta,s) )dF(s) dG_i(\theta)- r 
\end{align*}
...
\begin{align*}
        \pdv[2]{R^{IR}}{k} = &  \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \int_s  \pdv{u}{q}\pdv{q^m_{i,2}}{k} dF(s) dG_i(\theta)= -1
\end{align*}
...
\begin{align*}
     \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} q^m_{i,4}(\zeta,\varepsilon) dG_i(\theta) = k
\end{align*}

The derivative with respect to $k$ gives:

\begin{align}\label{eq:pdvq4k1}
    \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \left( \pdv{q^m_{i,4}}{\zeta}\pdv{\zeta}{k} +  \pdv{q^*_{i,4}}{\varepsilon}\pdv{\varepsilon}{k} \right)  dG_i(\theta) = 1
\end{align}
...
\begin{align*}
    \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} \frac{1}{1+\zeta} \left( J_i(q^m_{i,4},\theta,s)\pdv{\zeta}{k}  - \pdv{\varepsilon}{k} \right)  dG_i(\theta) = 1
\end{align*}
...
With $\mathbb{E}_\theta J_i(q^m_{i,4},\theta,s) = \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  J_i(q^m_{i,4},\theta,s)$ the aggregate virtual marginal utility across all consumers. We turn toward the $R^{IR}$ constraint:


\begin{align*}
    & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} \left[ \int_{0}^{s^m_1(k)}  (U(q^m_{i,3}(\zeta),\theta,s)  -\Gamma_i(\theta)q^m_{i,3}(\zeta) ) dF(s)  \right. \\
...
\begin{align}\label{eq:pdvq4k2}
    & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} \left[ \int_{0}^{s^m_1(k)}  \left( J(q^m_{i,3}(\zeta),\theta,s)\pdv{q^m_{i,3}}{\zeta} \pdv{\zeta}{k} \right) dF(s)  \right. \\
...
\begin{align*}
    & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} \left[ \int_{0}^{s^m_1(k)}  \left( \frac{ (J_i(q^m_{i,3},\theta,s))^2 }{1+\zeta}  \pdv{\zeta}{k} \right) dF(s)+   \int_{s^m_1(k)}^{\bar{s}}  \left( \frac{ (J(q^m_{i,4}(\zeta),\theta,s))^2}{1+\zeta}  \pdv{\zeta}{k}  \right) dF(s)  \right. \\
...
\begin{equation*}
    \mathbb{K} = r - \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} \int_{s^m_1(k)}^{\bar{s}}  J(q^m_{i,4}(\zeta),\theta,s)  dF(s)dG_i(\theta) 
\end{equation*}

and

\begin{equation*}
    \mathbb{M} =\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} \left[  \int_s (J_i(q^m_{i,l},\theta,s)))^2 dF(s)dG_i(\theta) - \int_{s^m_1(k)}^{\bar{s}}  ( J(q^m_{i,4},\theta,s)\mathbb{E}_\theta J_i(q^m_{i,4},\theta,s))dF(s)  \right]
\end{equation*}
...
First note that : $\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} J(q^m_{i,4},\theta,s)  dG_i(\theta) = s - k + \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i} $, this can seen from the fact that by definition $\sum_{i = \{1,2\}} \mu_i q^m_{i,4} = k$ and $\Gamma_i(\theta) = \hat{\theta}_i - \theta$ under uniform distribution. Then, we show that $\mathbb{K} > 0$ for every $k$ in the case that the capacity always binds (i.e., $s_1^m(k) = 0)$. Using the expression of the aggregate virtual marginal utility, we have $\mathbb{K} = k + r - \frac{\Bar{s}}{2} - \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i}$. Next, we use Proposition \ref{propch3:prop5} to show that it is always positive. Indeed, the maximum of the non-binding constraint $R^{IR}$ described in equation \ref{eq:RIR} and noted $k^{IR}$ is equal to $k^{IR} r - \frac{\Bar{s}}{2} - \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i}$. Hence we can rewrite the $\mathbb{K} = k - k^{IR}$. As $R^{IR}(k)$ is concave in $k$ and $R^{IR}(0)>0$, therefore the solution of $R^{IR}(k) = 0$ is always grater than the maximum $k^{IR}$. Because we study the case when $R-IR$ is binding, the level of investment of interest is such that $k$ is above the solution of $R^{IR}(k) = 0$. Hence, the $k$ of interest is also greater than $k^{IR}$. Therefore, $\mathbb{K}\geq 0$. 
First note that : $\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} J(q^m_{i,4},\theta,s)  dG_i(\theta) = s - k + \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i} $, this can seen from the fact that by definition $\sum_{i = \{1,2\}} \mu_i q^m_{i,4} = k$ and $\Gamma_i(\theta) = \hat{\theta}_i - \theta$ under uniform distribution. Then, we show that $\mathbb{K} > 0$ for every $k$ in the case that the capacity always binds (i.e., $s_1^m(k) = 0)$. Using the expression of the aggregate virtual marginal utility, we have $\mathbb{K} = k + r - \frac{\Bar{s}}{2} - \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i}$. Next, we use Proposition \ref{propch3:prop5} to show that it is always positive. Indeed, the maximum of the non-binding constraint $R^{IR}$ described in equation \ref{eq:RIR} and noted $k^{IR}$ is equal to $k^{IR} r - \frac{\Bar{s}}{2} - \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i}$. Hence we can rewrite the $\mathbb{K} = k - k^{IR}$. As $R^{IR}(k)$ is concave in $k$ and $R^{IR}(0)>0$, therefore the solution of $R^{IR}(k) = 0$ is always grater than the maximum $k^{IR}$. Because we study the case when $R-IR$ is binding, the level of investment of interest is such that $k$ is above the solution of $R^{IR}(k) = 0$. Hence, the $k$ of interest is also greater than $k^{IR}$. Therefore, $\mathbb{K}\geq 0$. 
First note that : $\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} J(q^m_{i,4},\theta,s)  dG_i(\theta) = s - k + \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i} $, this can seen from the fact that by definition $\sum_{i = \{1,2\}} \mu_i q^m_{i,4} = k$ and $\Gamma_i(\theta) = \hat{\theta}_i - \theta$ under uniform distribution. Then, we show that $\mathbb{K} > 0$ for every $k$ in the case that the capacity always binds (i.e., $s_1^m(k) = 0)$. Using the expression of the aggregate virtual marginal utility, we have $\mathbb{K} = k + r - \frac{\Bar{s}}{2} - \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i}$. Next, we use Proposition \ref{propch3:prop5} to show that it is always positive. Indeed, the maximum of the non-binding constraint $R^{IR}$ described in equation \ref{eq:RIR} and noted $k^{IR}$ is equal to $k^{IR} r - \frac{\Bar{s}}{2} - \sum_{i = \{1,2\}} \mu_i {\ubar{\theta}_i}$. Hence we can rewrite the $\mathbb{K} = k - k^{IR}$. As $R^{IR}(k)$ is concave in $k$ and $R^{IR}(0)>0$, therefore the solution of $R^{IR}(k) = 0$ is always grater than the maximum $k^{IR}$. Because we study the case when $R-IR$ is binding, the level of investment of interest is such that $k$ is above the solution of $R^{IR}(k) = 0$. Hence, the $k$ of interest is also greater than $k^{IR}$. Therefore, $\mathbb{K}\geq 0$. 
...
\begin{align*}
        \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \int_0^{s_1}  (U(q^m_{i,1},\theta,s) - U(q^m_{i,2},\theta,s)  -\Gamma_i(\theta)(q^m_{i,1} - q^m_{i,2}) ) dF(s) dG_i(\theta)
\end{align*}

As only the on-peak allocation $q^m_{i,2}$ depends on $k$, the first derivative with respect to $k$ gives:

\begin{align*}
        \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \int_0^{s_1}  (- u(q^m_{i,2},\theta,s)  + \Gamma_i(\theta)) dF(s) dG_i(\theta)
\end{align*}

Note that at $k^-$, the derivative is null. Finally, the second derivative with respect to $k$ gives:

\begin{align*}
 \sum_{i = \{1,2\}} \mu_i  \int_{\theta_i}\left[- \pdv{s_1}{k} \/ (u(q^m_{i,2}(\theta,s_1),\theta,s_1) -\Gamma_i(\theta) ) f(s) +   \int_0^{s_1}  1 dF(s)   \right]dG_i(\theta) 
\end{align*}
...
\begin{align*}
    \mathbb{M} = & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \int_s (J_i(q^m_{i,2},\theta,s))^2 dF(s)dG_i(\theta) \\
   + & \sum_{i = \{1,2\}} \mu_i \int_{\theta_i}  \int_{s^m_1(k)}^{\Bar{s}} ( (J_i(q^m_{i,4},\theta,s))^2  - J(q^m_{i,4},\theta,s)\mathbb{E}_\theta J_i(q^m_{i,4},\theta,s))dF(s) dG_i(\theta)
\end{align*}
The first term is positive. Dropping the reference to the state of the world $s$, the second term can be rewritten as : 

\begin{equation*}
     \sum_{i = \{1,2\}} \mu_i \int_{\theta_i} (J_i(q^m_{i,4},\theta,s))^2 dG_i(\theta) - \left[\sum_{i = \{1,2\}} \mu_i \int_{\theta_i} J(q^m_{i,4},\theta,s){\theta_i}dG_i(\theta)  \right]^2
\end{equation*}
...
\begin{equation*}
    \theta^{m}_i = \frac{1}{\mathbb{A}}(1+\zeta) - \Bar{\theta}_1 + \sum_{i = \{1,2\}} (\Bar{\theta}_i - \ubar{\theta}_i)
\end{equation*}
