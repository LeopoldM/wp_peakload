...
\begin{lemma}\label{lem:suffstat}
  The sign of\, $\mathbb{E}_{w_i}[uJ\mid s]$ simultaneously determines:
...
This object depends on two dimensions: (i)~how the additional capacity is shared between types, captured by $R_q(\theta)$, and (ii)~how the marginal virtual surplus changes across types, captured by $R_\theta(\theta)$. The interaction between these two dimensions is formalised by the following condition:
\begin{equation}
...
We now characterise how the sign of $\mathbb{E}_{w_i}[uJ\mid s]$ evolves with $k_i$. The key primitive is whether the capacity sorting is \textit{submodular}: capacity expansion disproportionately benefits lower types.

\begin{definition}
  $R_q$ is \emph{submodular in $(\theta,k_i)$} if $\partial^2 R_q/\partial\theta\,\partial k_i\leq 0$
\end{definition}

\begin{lemma}[Single-crossing of the sufficient statistic]\label{lem:SC}
  Assume $R_\theta(\theta)$ is increasing and $R_q$ is submodular in $(\theta,k_i)$. Then $\mathbb{E}_{\theta_i}[R_\theta(\theta)R_q(\theta)]$ is single-crossing in $k_i$: there exists $\bar k_i$ such that $\mathbb{E}_{\theta_i}[R_\theta R_q]\geq0$ for $k_i\leq\bar k_i$ and $\mathbb{E}_{\theta_i}[R_\theta R_q]\leq0$ for $k_i\geq\bar k_i$.
\end{lemma}

The economic interpretation of submodularity is intuitive. When capacity is scarce, the designer allocates relatively uniformly across types; as $k_i$ expands, the incentive-compatible allocation is going disproportionately to lower types (as $R_q(\theta)$ increases more at the bottom). Combined with $R_\theta(\theta)$ increasing --- lower types have lower marginal virtual surplus --- the covariance term in \eqref{COV1} becomes increasingly negative as $k_i$ grows. At low capacity, the average effect $\mathbb{E}[R_\theta]\mathbb{E}[R_q]$ dominates and the sign of the sufficient statistic is positive; at high capacity, the adverse sorting effect dominates and it becomes negative. Lemma~\ref{lem:SC} formalises this single-crossing intuition.

\begin{remark}[Primitives for submodularity]\label{rem:prim} We show in the proof of Lemma~\ref{lem:SC} that the submodularity of $R_q$ in $(\theta,k_i)$ is equivalent to the expression $R_q(\theta)$ being concave with respect to $q$: 
  \[
    \frac{\partial^2 R_q(\theta)}{\partial \theta \partial k} \le 0 \quad\Leftrightarrow \quad \frac{\partial^2 R_q(\theta)}{\partial^2 q} \le 0  .
  \]
It is satisfied by many standard utility specifications, including linear utility, exponential (CARA), power/isoelastic (CRRA), and, more generally, the HARA family.
\end{remark}


We discuss in Section~\ref{sec:redist} the monotonicity condition of $R_\theta(\theta)$, The sign of its derivative is governed by the distribution of types $g_i(\theta)$ as well as by the shape of the preference weights $\lambda_i(\theta)$.
...
\end{equation}
Two observations about \eqref{partialkCS-boundary} are immediate. First, its sign is governed by $(\operatorname{COV}_{q\theta}^\times)$, as established in Lemma~\ref{lem:suffstat}. Second, when \eqref{Rmdi-def} binds (the IR-constrained allocation), the boundary term equals zero: the entire welfare change is then carried by the IC rent in \eqref{partialkCS1}.

\begin{theorem}\label{thm:welfare}
  Whenever it exists, define a cutoff $\tilde\theta_i^1\in [\underline\theta_i,\overline\theta_i]$ by $\partial\mathbb{E} CS_i^\times(\tilde\theta_i^1)/\partial k_i=0$.

  \medskip
  \noindent\textbf{IR-unconstrained allocation.} (i)~If \eqref{COV1} holds, then an increase in $k_i$ increases surplus for every type $\theta$. (ii)~If \eqref{COV1} is reversed, then $\tilde\theta_i^1$ is unique; individual surplus decreases for all $\theta<\tilde\theta_i^1$ and increases for all $\theta>\tilde\theta_i^1$.

  \medskip
  \noindent\textbf{IR-constrained allocation} (characterised in Proposition~\ref{prop:implementation} below). Assume $R_\theta(\theta)$ is monotone. If $R_\theta(\theta)$ is increasing, then $\tilde\theta_i^1$ is unique and:
  \begin{enumerate}[label=(\roman*), leftmargin=1cm]
    \item if \eqref{COV1} holds, individual surplus decreases for all $\theta>\tilde\theta_i^1$ and increases for all $\theta<\tilde\theta_i^1$;
    \item if \eqref{COV1} is reversed, individual surplus decreases for all $\theta<\tilde\theta_i^1$ and increases for all $\theta>\tilde\theta_i^1$.
  \end{enumerate}
  If $R_\theta(\theta)$ is decreasing, the inequalities are reversed.
\end{theorem}

Expanding capacity is not always surplus-improving for all consumers, even when the budget requirement does not bind (IR-unconstrained allocation) or for a fixed budget (IR-constrained allocation). Which types benefit from an increase in $k_i$ depends on two objects: (i)~the direction of the boundary term, governed by $(\operatorname{COV}_{q\theta}^\times)$, and (ii)~the shape of $R_\theta(\theta)$, which captures the trade-off between revenue extraction and incentive constraints.

When the participation constraint is slack ($\beta_i^\times=0$), the second term in parentheses in \eqref{partialkq1} vanishes, so the optimal allocation is increasing in $k_i$ for all types during on-peak states and constant off-peak. The boundary term in \eqref{partialkCS1} then corresponds to an equal change in surplus across all types, while the IC rent captures the additional incentive rents induced by the change in $q_i^\times$. If $(\operatorname{COV}_{q\theta}^\times)$ holds, both terms are positive: the common component and the IC rent both increase with $k_i$, so an increase in $k_i$ improves the welfare of all consumers. If $(\operatorname{COV}_{q\theta}^\times)$ is reversed, lower types are harmed. Indeed, the cross-partial derivative satisfies
\[
...
Under the IR-constrained allocation (i.e., when \eqref{Rmdi-def} binds), the allocation must be distorted so that expected revenue equals the budget. This distortion is captured by the second term in parentheses in \eqref{partialkq1}. We show in the proof that $\beta_i^\times$ is differentiable in $k_i$ on the IR-constrained region, and that the sign of $\partial\beta_i^\times/\partial k_i$ is governed by the sign of \eqref{Eujmd}, which is equivalently determined by $(\operatorname{COV}_{q\theta}^\times)$. The bracketed term in \eqref{partialkq1} is the deviation of a type's marginal virtual surplus $u(q_i^\times(\theta,s),s)J_i(\theta)$ from its $w_i^\times$-weighted average; its sign is therefore determined by the relative ranking of types according to marginal virtual surplus, which is captured by $R_\theta(\theta)$. If $R_\theta(\theta)$ is monotone, this deviation changes sign at most once, yielding a unique cutoff and the type-ranking described in Theorem~\ref{thm:welfare}. To interpret the condition, suppose expanding $k_i$ makes the budget less tight, so that $\beta_i^\times$ decreases in $k_i$. The need for revenue extraction then falls, and the designer lowers the marginal weight on types that generate higher marginal virtual surplus. The sign of $R_\theta(\theta)$ pins down which types those are.

%------------------------------------------------------------
\subsubsection{Implementation region}\label{sec:impl}
%------------------------------------------------------------

The single-crossing result in Lemma~\ref{lem:SC} has an immediate implication for the implementation region: the capacity levels for which the IR-unconstrained allocation (i.e., \eqref{Rmdi-def} slack, $\beta_i^\times=0$) is feasible and characterised by at most two thresholds.

\begin{proposition}[Implementation region]\label{prop:implementation}
Fix category $i$ and budget requirement $I_i > 0$. Assume $R_\theta(\theta)$ is nondecreasing on $[\underline\theta_i,\overline\theta_i]$ and $R_q''(q)\le 0$ (concave risk tolerance). Define $k^+_i$ as the capacity level at which the capacity never binds. Then
\begin{enumerate}[label=(\roman*), leftmargin=1cm]
    \item If $\mathbb{E}\underline{CS}^\times_i(k^+_i) \geq 0$, there exists a unique cutoff $\tilde k_i \in (0,k^+_i]$ such that the IR-unconstrained allocation is optimal for $k_i \geq \tilde k_i$. Moreover, this holds whenever $J_i(\theta) \geq 0$ for all $\theta$.
    \item If $\mathbb{E}\underline{CS}^\times_i(k^+_i) < 0$, the IR-unconstrained allocation is either never optimal or optimal only on an intermediate interval on $[0, k^+_i]$.
\end{enumerate}
\end{proposition}

The threshold $\tilde k_i$ defines when the market designer can finance the investment $I_i$ without distorting the allocation via the IR constraint. The condition follows directly from the single-crossing of $\mathbb{E}[uJ]$ in $k_i$ established in Lemma~\ref{lem:SC}: as capacity expands and adverse sorting worsens, the expected revenue that the mechanism can sustain first increases and then falls. The intersection of this revenue curve with $I_i$ pins down the implementation threshold. In general, $(\operatorname{COV}_{q\theta}^\times)$ does not hold pointwise in $s$: because of the capacity constraint, the optimal allocation varies with $s$, and the aggregate effect depends on how the virtual term $J_i(\theta)$ is sorted across types as $s$ changes.

The following corollary provides functional-form conditions under which the sign of $(\operatorname{COV}_{q\theta}^\times)$ is independent of $(s,k_i)$. It isolates the distribution of virtual surplus $J_i(\theta)$ relative to social weights $\Gamma_i(\theta)$ as the sole determinant of the implementation region, confirming that the key primitive is how revenue generation is distributed across types rather than the level or state of the capacity allocation.
...
We illustrate in Figure~\ref{fig:impl} the sensitivity of the implementation regions with respect to the model primitives. The first panel shows the expression from Corollary~\ref{cor:sep} with respect to the lower bound of the support $\Theta_i$: a lower $\underline\theta_i$ shifts more mass toward types with negative virtual surplus. The second panel shows $(\operatorname{COV}_{q\theta}^\times)$ with respect to the capacity level $k_i$ for $\underline\theta_i$ on either side of the threshold identified in Lemma~\ref{lem:SC}. In the upper plot, the mass of consumers with negative $J_i(\theta)$ is relatively low, so the boundary term is increasing for all $k_i$ and $s$, and the IR-unconstrained region is located in the high $k_i$ region. The opposite is observed in the lower plot.
