...
\begin{remark}[Primitive foundation in special cases]\label{rem:prim}
The log-monotone-differences condition in Lemma~\ref{lem:SC-dir} is, in general, a restriction on the endogenous equilibrium object \(R_q(\theta,k_i)\), not solely on primitives of \(U\). On the IR-unconstrained region, where \(\beta_i^\times=0\), the comparative statics simplify and the sign of \(\partial_{\theta k_i}\log R_q\) can be related to the curvature of the primitive map \(q\mapsto R_q(q,s)\). By contrast, on the IR-constrained region, the response of the shadow cost \(\beta_i^\times(k_i)\) enters the derivative and the condition is no longer implied by curvature of \(U\) alone.

A useful benchmark is CARA utility. In that case, \(R_q(q,s)\) is constant in \(q\), so \(\partial_{\theta k_i}\log R_q(\theta,k_i,s)=0\). Hence, the log-monotone-differences condition is trivially satisfied.
\end{remark}
...
\begin{definition}\label{ass:multiplicative}
$u(\cdot)$ is multiplicative in $(q,s)$ if there exists two functions $\kappa(q)$ and $\zeta(s)$ such that $u(q,s) = \kappa(q) \zeta(s)$. Add conditions on $\kappa$ and $\zeta$.
\end{definition}
...
\noindent\textbf{IR-constrained allocation.} Assume \(R_\theta(\theta)\) is monotone and \(u(\cdot)\) is multiplicative in \((q,s)\). The lowest type always lies on the boundary: \(\underline\theta_i\in\tilde\Theta_i\). If \(R_\theta(\theta)\) is increasing, a second cutoff \(\tilde\theta_i\in(\underline\theta_i,\overline\theta_i)\) may exist and when it does:
