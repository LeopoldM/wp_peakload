...
We define $S^*=[\underline{s},s^*)$ and $T^*=[s^*,\overline{s}]$ as the off-peak and on-peak sets, where $s^*$ is the unique threshold at which $\sum_{i} \mu_i \E_{\theta_i} [q_i^*(\theta,s^*)] = k$. The marginal welfare weight $\lambda^*=\beta$ is pinned down by the budget constraint:
\[
\sum_i \mu_i \E_{\{s,\theta_i\}}\left[\theta\, U(q_i^*(\theta,s),s) \textbf{1}_{\lambda_i(\theta) \leq \lambda^*}\right] = I(k).
\]
\begin{proposition}\label{prop:FBalloc}
