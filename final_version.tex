...
\begin{equation}
  \mathbb{E}_{\theta_i}\!\left[R_\theta(\theta)R_q(\theta)\right] = \mathbb{E}_{\theta_i}[R_\theta(\theta)]\,\mathbb{E}_{\theta_i}[R_q(\theta)] + \operatorname{Cov}_{\theta_i}\!\left(R_\theta(\theta),R_q(\theta)\right) \geq 0. \tag{signEuJ}\label{COV1}\
\end{equation}
...
The following corollary provides functional-form conditions under which the sign of \eqref{Eujmd} is independent of $s$. It isolates the distribution of virtual surplus $J_i(\theta)$ relative to social weights $\G_i(\theta)$ as the sole determinant of the implementation region, confirming that the key primitive is how revenue generation is distributed across types.
