...
\begin{titlepage}
\externaldocument{final_version}
...
\eqref{Eujmd} is a sufficient statistic for the comparative statics of the within-category problem. In particular, its sign provides a common sign benchmark for:
\begin{enumerate}[label=(\roman*), leftmargin=1cm]
    ...
    \item the reallocation component of $\partial q_i^\times(\theta,s)/\partial k_i$.
\end{enumerate}
\end{lemma}
...
\begin{figure}
    ...
    \caption{Effects of change in redistributive preferences on the trade-off effect. The first plot shows the new $\lambda_i(\theta)$. The second plot indicates the effect of the perturbation on the term $\Delta R_\theta(\theta)$. The last plot links the condition in Proposition \ref{prop:PrefTradeoff} to the sign of $\Delta R'_\theta(\theta)$. $R_q(\theta)$ is assumed to be decreasing in $\theta$, $R_\theta(\theta)$ and $J_i(\theta)$ are increasing in $\theta$. In this example, $R'_\Gamma(\theta)<0$ and $J_i(\theta)>0$ for all $\theta$, so the worsening effect comes from types generating revenues while receiving proportionally decreasing weights.}
