...
Assume \(R_\theta(\theta)\) is monotone in \(\theta\) and \(R_q(\theta,k_i)\) has monotone differences in \((\theta,k_i)\) (i.e., it is either submodular or supermodular). Then the map
%##########################################
\[
k_i \longmapsto \mathbb{E}_{w_i}[uJ\mid s]
\]
%##########################################
is single-crossing. Moreover, the direction is pinned down by the combination of monotonicities: it crosses from \(+\) to \(-\) when \(R_\theta\) is increasing and \(R_q\) is submodular, or when \(R_\theta\) is decreasing and \(R_q\) is supermodular; it crosses from \(-\) to \(+\) in the two remaining cases.
\end{lemma}
...
%##########################################
Then, for any sign configuration of $(R_\Gamma(\theta), R_\theta'(\theta), R_q'(\theta))$ such that $\mathrm{sign}(-R_\Gamma(\theta)R_\theta'(\theta))$ aligns with the sign of $\Delta R_\theta'(\theta)$,
%##########################################
\[\mathrm{sign}\underbrace{\left(\Cov_{\theta_i}\!\left(\Delta R_\theta(\theta),R_q(\theta)\right)\right)}_\text{trade-off sorting effect} = \mathrm{sign}\!\left(-R_\Gamma(\theta) R_\theta'(\theta)\right) \cdot \mathrm{sign}\!\left(R_q'(\theta)\right).\]
%##########################################
In sign configurations where $\mathrm{sign}(-R_\Gamma(\theta)R_\theta'(\theta))$ does not align with the sign of $\Delta R_\theta'(\theta)$, the elasticity condition is necessary but not sufficient to sign the covariance term.
\end{proposition}
...
%##########################################
\[
\Delta q_i^\times(\theta,s) = R_q(\theta,s)\!\left(R_{\G}(\theta) - \mathbb{E}^w_i\!\left[R_{\G}\mid s\right]\right), \qquad s\in T_i^\times.
\]
%##########################################
\end{lemma}
