...
\mathbb E_{w_i}[uJ\mid s]\right]. \tag{EuJ}\label{Eujmd}
\end{equation}
%##########################################
...
\mathbb E_{w_i}[uJ\mid s]\mathbf 1_{\{s\in T_i^\times\}}\right]
\]
...
\noindent\textbf{IR-unconstrained allocation.} If \eqref{Eujmd} is positive, then an increase in \(k_i\) increases expected surplus for every type \(\theta\). If \eqref{Eujmd} is negative, \(\tilde\Theta_i\) contains a unique cutoff; expected surplus decreases for all \(\theta<\tilde\theta_i\) and increases for all \(\theta>\tilde\theta_i\).

\noindent\textbf{IR-constrained allocation.} Assume \(R_\theta(\theta)\) is monotone and Assumption~\ref{ass:mult} holds. The lowest type always lies on the boundary: \(\underline\theta_i\in\tilde\Theta_i\). If \(R_\theta(\theta)\) is increasing, a second cutoff \(\tilde\theta_i\in(\underline\theta_i,\overline\theta_i)\) may exist and when it does:
...
\noindent i) Then for all $\theta\in[\underline{\theta}_i,\overline{\theta}_i]$,
%##########################################
        \[
