...
% The first regime we study is the complete information case: the market designer observes each consumer's category $i$ and type $\theta$, so the individual welfare weight $\lambda_i(\theta)$ is known and allocation rules can condition on true types without incentive issues. The category label $i$ remains relevant as it indexes the population mass $\mu_i$ and the type distribution $G_i(\theta)$, but the average weight $\tilde{\lambda}_i$ plays no allocative role: what matters for redistribution is the pointwise value $\lambda_i(\theta)$.

% For each realization of the shock $s$, we define the allocation under complete information with $q^*_i(\theta, s)$ that maps the observed type of each consumer in category $i$ to the quantity allocated. The monetary transfer $t^*_i(\theta,s)$ maps the observed type to the payment made by the consumer to the market designer. We derive the optimization problem as follows, with the objective function corresponding to the weighted expected sum of consumer surpluses across types and states:
% \[
% \max_{\substack{ t_i(\theta,s) \geq 0, \\ q_i(\theta,s) \geq 0}} \quad \sum_{i} \mu_i \; \E_{(s,\theta_i)} \left[\lambda_i(\theta)\;\theta \; U(q_i(\theta,s),s) - t_i(\theta,s) \tag{P-CI}\label{P-CI}\right]
% \]

% The allocation mechanism is budget-feasible and feasible if: the constraint \eqref{R-def} is satisfied (revenues must cover investment costs $I(k)$, given zero marginal production costs), the capacity constraint \eqref{K-def} is satisfied for all states $s$, and every consumer of type $(\theta,i)$ has nonnegative expected utility as per the participation constraint \eqref{IR-def}. With multipliers $\varepsilon(s) \geq 0$ (capacity), $\beta\geq0$ (budget), $\rho_i(\theta)\geq0$ (participation constraint), and $\phi_i(\theta,s)\geq 0$ (transfer positivity), the stationary conditions are
% \begin{align}
% \label{KKT-q}\tag{FOC$_q^*$}
% \left(\mu_i\lambda_i(\theta)+\rho_i(\theta)\right)\theta \;u\!\left(q_i(\theta,s),s\right) - \mu_i\varepsilon(s) = 0,
% \\
% \label{KKT-t}\tag{FOC$_t^*$}
% -\mu_i\lambda_i(\theta) + \mu_i\beta - \rho_i(\theta) + \phi_i(\theta,s) = 0.
% \end{align}
% We show in Proposition~\ref{prop:FBalloc} that under a general mechanism, the optimal allocation is defined by a unique quantity schedule $q_i^*(\theta,s)$ and transfers are pinned down only at the type level. Let $s^*:= s^*(k)$ denote the first state in which the capacity constraint binds:
% \[
% \sum_{i} \mu_i \E_{\theta_i} [q_i^*(\theta,s^*)] = k.
% \]
% We define $S^*=[\underline{s},s^*)$ as the set of off-peak states (capacity slack) and $T^*=[s^*,\overline{s}]$ as the set of on-peak states (capacity binding). The marginal welfare weight $\lambda^*$ associated with the investment cost $I(k)$ is defined by the budget constraint:
% \[
% \sum_i \mu_i \E_{\{s,\theta_i\}}\left[\theta\, U(q_i^*(\theta,s),s) \textbf{1}_{\lambda_i(\theta) \leq \lambda^*}\right] dG_i(\theta) = I(k).
% \]


% \begin{proposition}\label{prop:FBalloc}
% The optimal allocation is characterized by a unique quantity schedule $q_i^*(\theta,s)$ for each category $i$, type $\theta$, and state $s$:
% \[
...
% \qquad s\in T^{\ast},
% \]
% where $\varepsilon(s)\ge 0$ and $\beta\ge 0$ are the Lagrange multipliers on the capacity and budget constraints, and $\lambda^* = \beta$ is the marginal welfare weight pinned down by the budget constraint. Moreover, the optimal transfers are independent of $s$ and satisfy $t_i^*(\theta) > 0$ if $\lambda_i(\theta) \leq \lambda^*$ and $t_i^*(\theta) = 0$ if $\lambda_i(\theta) > \lambda^*$.
% \end{proposition}


% The results follow from the Lagrangian of program \eqref{P-CI} and generalize the Boiteux-Steiner framework (see \citealp{crew1995theory}) to redistributive weights. We derive the optimal allocation under uniform weights in the Online Appendix. Off-peak ($s\in S^*$), the optimal allocation reproduces marginal-cost pricing: $\theta\, u(q_i^*(\theta,s),s)=0$, ensuring that each consumer's marginal willingness to pay equals the marginal production cost. On-peak ($s\in T^*$), the optimal allocation is governed by the threshold $\lambda^*=\beta$. Any type $(\theta,i)$ with $\lambda_i(\theta)\leq\lambda^*$ contributes to the budget: the participation constraint binds and the market designer sets $t_i^*(\theta)>0$ to extract the full expected surplus. Any type with $\lambda_i(\theta)>\lambda^*$ receives a zero transfer and its IR constraint is slack. The threshold $\lambda^*$ is pinned down by the budget constraint as the marginal welfare weight of the last contributing type.

% This threshold structure induces two distinct on-peak allocation rules. For contributing types ($\lambda_i(\theta)\leq\lambda^*$), capacity is allocated such that the weighted marginal willingness to pay is equalized across all consumers at $\varepsilon(s)/\beta$: the market designer distorts the allocation upward relative to the unconstrained optimum to relax the binding participation constraint and extract more revenue. For non-contributing types ($\lambda_i(\theta)>\lambda^*$), the optimal allocation satisfies $\theta\, u(q_i^*(\theta,s),s)=\varepsilon(s)/\lambda_i(\theta)$, so the capacity share is increasing in $\lambda_i(\theta)$: for a given $\theta$, if $\lambda_i(\theta)>\lambda_{i'}(\theta')$ then $q_i^*(\theta,s)>q_{i'}^*(\theta',s)$, since $u_q<0$.

% A spot market fails to implement this allocation whenever welfare weights are heterogeneous. Under a spot market, a uniform price $p(s)$ equates marginal willingness to pay across all consumers: $\theta\, u(q,s)=p(s)$ for all $(i,\theta)$. This coincides with the optimum only in the knife-edge case where all types are contributing and $\lambda_i(\theta)=\beta$ uniformly. For non-contributing types, the optimal condition $\theta\, u(q_i^*,s)=\varepsilon(s)/\lambda_i(\theta)$ varies with $\lambda_i(\theta)$, which a single price $p(s)$ cannot replicate. This stands in contrast to the uniform-weight case, where $\lambda_i(\theta)=\lambda$ for all $(i,\theta)$ and the spot market allocation is optimal regardless of whether IR constraints bind.

% Without additional structure, the sign of $\partial q_i^*/\partial k$ when capacity is binding is generically ambiguous, which limits the analysis of surplus changes. Define the weight for each type $(\theta,i)$ as 
% \[w_i^c(\theta,s) := -\frac{1}{\lambda_i(\theta)\,u_q(q_i^*,s)} \qquad \text{and} \qquad w_i^{nc}(\theta,s) := -\frac{1}{\beta\,u_q(q_i^*,s)}\]
% for respectively contributing and non contributing types, with $\bar w^x_i(s) := \mathbb{E}_{\theta_i}[w_i^x(\theta,s)]$ and $x=\{c,nc\}$. These weights are positive since $u_q<0$. Define the weighted expected marginal utility over on-peak periods as
% \[
% \mathbb{E}_w[u^x] := \mathbb{E}_s\left[\sum_{i} \mu_i  \frac{1}{\overline{w}^x_i(s)}\E_{\theta_i} [w_i^x(\theta,s) \,\theta\, u(q_i^*(\theta,s),s)\mathbf{1}_{\lambda_i(\theta)\leq\lambda^*}]\,\mathbf{1}_{s\in T^*}\right].
% \]

% With $\mathbb{E}_w[u^\ast] = \mathbb{E}_w[u^c]  + \mathbb{E}_w[u^{nc}]  $

% \begin{proposition}\label{prop:CI-welfare}
% \textbf{i)} For any interval $k\subseteq\mathbb{R}_+$ such that $\lambda^*$ is constant, and the IR constraint of marginal types is slack, the individual surplus is (weakly) increasing in $k$ for all non-contributing types $\lambda_i(\theta)>\lambda^*$. The surplus for the marginal type $\lambda^\ast$ increases if and only if
% \[
% \mathbb{E}_w[u^c] \geq I'(k). \tag{Ui*}\label{Ui*}
% \]
% \textbf{ii)} If the IR constraint of marginal types binds and \eqref{Ui*} holds, then the surplus of all non-contributing types is (weakly) increasing in $k$.
% \end{proposition}

% A key feature of Proposition~\ref{prop:CI-welfare} is that welfare effects are uniform within each group: all non-contributing types move in the same direction simultaneously, and all contributing types likewise. There is no within-group cutoff $\tilde\theta_i$ separating winners from losers. This uniformity reflects the absence of IC rents under complete information: when the market designer observes $\theta$ directly, he extracts full surplus from contributing types, and the only margin that matters for investment is the aggregate budget condition \eqref{Ui*}, not the distribution of surplus within a group.

% Condition \eqref{Ui*} has a clean interpretation: it requires that the weighted marginal benefit of capacity expansion across contributing types exceeds the marginal investment cost. When it holds, the market designer can self-finance the increase in $k$ from contributing types without tightening the allocation of non-contributing types, so $\partial\varepsilon(s)/\partial k < 0$ and all non-contributing types gain. When it fails, the additional investment cost cannot be covered without distorting allocations, and surplus may fall even for non-contributing types.

% This benchmark result will be modified along two precise dimensions when we introduce incomplete information in Section~\ref{chap3sec:sec5}. First, the market designer can no longer extract full surplus from contributing types: IC constraints force him to leave information rents, so the effective budget available to self-finance investment is the \textit{virtual surplus} rather than the full surplus. Second, and more fundamentally, within-group heterogeneity reappears: the distribution of information rents across types within a group creates a cutoff $\tilde\theta_i$ such that low types may lose from capacity expansion even when high types gain. The covariance condition that governs this cutoff is the central object of the analysis in Section~\ref{chap3sec:sec5}.

% We conclude in the following corollary the effect of a change in the weight distribution. 

% \begin{corollary}

% Consider a change in social weights $\lambda_j$ such that for a given $k_i$: $j^*$ remains the marginal category, $d\lambda_j<0$ for all $j<j^*$, $d\lambda_{j^*}=0$, $d\lambda_j>0$ for all $j>j^*$ and $\sum_j d\lambda_j = 0$. Then, if $u$ is linear in $q$, condition \eqref{Ui*} becomes less stringent.
...
Let $S^*=[\underline{s},s^*)$ and $T^*=[s^*,\overline{s}]$ be the off-peak and on-peak sets, where $s^*$ is the unique threshold at which $\sum_{i} \mu_i \E_{\theta_i} [q_i^*(\theta,s^*)] = k$. Define the marginal welfare weight such that the utility from the optimal allocation for all types with lower social weights cover the investment cost:
\[
\sum_i \mu_i \E_{\{s,\theta_i\}}\left[\theta\, U(q_i^*(\theta,s),s) \textbf{1}_{\lambda_i(\theta) < \lambda^*}\right] < I(k) \le \sum_i \mu_i \E_{\{s,\theta_i\}}\left[\theta\, U(q_i^*(\theta,s),s) \textbf{1}_{\lambda_i(\theta) \leq \lambda^*}\right].
\]
\begin{proposition}\label{prop:FBalloc}
...
\qquad s\in T^{\ast}.
\]
The optimal transfers are independent of $s$ and satisfy $t_i^*(\theta) > 0$ if $\lambda_i(\theta) \leq \lambda^*$ and $t_i^*(\theta) = 0$ if $\lambda_i(\theta) > \lambda^*$.
\end{proposition}
...
We now turn to the investment margin. Define the weight for each type $(\theta,i)$ as 
\[w_i^c(\theta,s) := -\frac{1}{\lambda_i(\theta)\,u_q(q_i^*,s)} \qquad \text{and} \qquad w_i^{nc}(\theta,s) := -\frac{1}{\beta\,u_q(q_i^*,s)}\]
for respectively contributing and non contributing types, with $\bar w^x_i(s) := \mathbb{E}_{\theta_i}[w_i^x(\theta,s)]$ and $x=\{c,nc\}$. These weights are positive since $u_q<0$. Define the weighted expected marginal utility over on-peak periods as
\[
\mathbb{E}_w[u^x] := \mathbb{E}_s\left[\sum_{i} \mu_i  \frac{1}{\overline{w}^x_i(s)}\E_{\theta_i} [w_i^x(\theta,s) \,\theta\, u(q_i^*(\theta,s),s)\mathbf{1}_{\lambda_i(\theta)\leq\lambda^*}]\,\mathbf{1}_{s\in T^*}\right].
...
\begin{proposition}\label{prop:CI-welfare}
\textbf{i)} For any interval $k$ such that $\lambda^*$ is constant, and the IR constraint of marginal types is slack, the individual surplus is (weakly) increasing in $k$ for all non-contributing types $\lambda_i(\theta)>\lambda^*$. The surplus for the marginal type $\lambda^\ast$ increases if and only if
\[
\mathbb{E}_w[u^c] \geq I'(k). \tag{Ui*}\label{Ui*}
\]
\textbf{ii)} If the IR constraint of marginal types binds and \eqref{Ui*} holds, then the surplus of all non-contributing types is (weakly) increasing in $k$.
...
\[\sign \; \frac{\partial}{\partial\theta}\left(\frac{\partial q^\times_i(\theta,s)}{\partial k}\right) = \sign \;  R_q'(q) \qquad \text{and} \qquad \sign \; \frac{\partial}{\partial\theta}\left(u(q_i^\times((\theta,s) J_i(\theta)\right) = \sign \;  R_\theta'(q)\]}: if $R_\theta(\theta)$ is increasing in $\theta$, then the marginal virtual surplus $u(q_i^\times,s)J_i(\theta)$ tends to be lower for lower types. Define the weights
\[
...
\end{equation}
The following lemma establishes that this weighted average marginal virtual surplus is the sufficient statistic that simultaneously governs the revenue effect of capacity expansion, the sign of the budget multiplier, and the marginal change in the optimal allocation.
...
In particular, those objects govern the marginal welfare and redistributive aspect of a capacity extension (in Theorem~\ref{thm:welfare}), the feasibility of the optimal allocation (Proposition~\ref{prop:implementation}).
...
Then the map
\[
k_i \longmapsto \mathbb{E}_{w_i}[uJ\mid s]
\]
...
Two observations: when \eqref{Rmdi-def} binds (the IR-constrained allocation), the boundary term equals zero: the entire welfare change is then carried by the IC rent in \eqref{partialkCS1}. On the other,  when \eqref{Rmdi-def} is slack (the IR-unconstrained allocation), then the second term in \eqref{partialkq1} is null as $\beta_i^\times = 0$.
...
\begin{theorem}\label{thm:welfare}
  ...
  \medskip
  \noindent\textbf{IR-unconstrained allocation.} (i)~If \eqref{Eujmd} is positive for every $s$, then an increase in $k_i$ increases surplus for every type $\theta$. (ii)~If \eqref{Eujmd} is negative, then $\tilde\theta_i$ is unique; individual surplus decreases for all $\theta<\tilde\theta_i$ and increases for all $\theta>\tilde\theta_i$.
  ...
  \begin{enumerate}[label=(\roman*), leftmargin=1cm]
    \item if \eqref{Eujmd} is positive for every $s$, individual surplus decreases for all $\theta>\tilde\theta_i$ and increases for all $\theta<\tilde\theta_i$;
    \item if \eqref{Eujmd} is negative for every $s$, individual surplus decreases for all $\theta<\tilde\theta_i$ and increases for all $\theta>\tilde\theta_i$.
  \end{enumerate}
...
In general, the sign condition \eqref{signEuJ} cannot be verified to be holding state-by-state: because the optimal allocation varies with $s$ through the capacity constraint, the sign of $\mathbb{E}_{\theta_i}[R_\theta(\theta)R_q(\theta)]$ at a given $s$ depends on the realized sorting across types, which changes with $s$. A state-independent sufficient condition is available only under the additional structure imposed in Corollary~\ref{cor:sep}. It isolates the distribution of virtual surplus $J_i(\theta)$ relative to social weights $\G_i(\theta)$ as the sole determinant of the implementation region, confirming that the key primitive is how revenue generation is distributed across types.
...
\begin{corollary}\label{cor:sep}
  ...
  \[
    \mathbb{E}_{\theta_i}\!\left[\frac{J_i(\theta)}{\G_i(\theta)^2}\right],
  \]
...
We illustrate in Figure~\ref{fig:redistribution_marg_k_uniform} the results from Theorem~\ref{thm:welfare} and Proposition~\ref{prop:implementation} as well as the the sensitivity of the implementation regions with respect to the model primitives. The first panel shows the value of the boundary term \eqref{ECSi} with respect to $k_i$ for two different types support. It represents both the implementation region of the optimal allocation as well as the regions of the sign of \eqref{Eujmd}. As a key element of this analysis is the existence of types with negative marginal virtual surplus, we show how an increase of the mass of such consumer affects the outcome of the model. To do so, the second support has a lower $\underline{\theta}_i$ which mechanically increase the mass of consumers having $J_i<0$. As expected this reduces the occurrence of capacity level for which\eqref{Eujmd}$>0$.\footnote{Intuitively, this reduces $k^\ast$ which reduces the progressivity of an increase of $k_i$ for lower types ($k^\ast$ on Figure~\ref{fig:impl_welfare} moves to the left).} The second panel with the set of plots illustrate how the progressivity/regressivity of the a capacity extension depends on the level of investment. The left panel shows 2 initial investment level ($k_1$ and $k_3$) and their respective deviation ($k_2$ and $k_4$). For initially low value ($k_1$) all consumers benefits from an increase in $k_i$ (when \eqref{Eujmd}$>0$ under the IR unconstrained), while for higher values of $k_i$ ($k_3$, when \eqref{Eujmd}$<0$), lower types below the threshold $\tilde\theta_i$ experienced a loss of surplus while all type above increases their surplus.
...
The comparative statics in this section examine how changes in redistributive preferences affect the sufficient statistic identified in Lemma~\ref{lem:suffstat}, through two channels: (we formally define below the preference-perturbation operator $\Delta(\cdot)$):
\[
...
$\Delta\G_i(\theta):=\G_i^\Delta(\theta)-\G_i(\theta)$.
Suppose $\G_i(\theta),\G_i^\Delta(\theta)>0$ for all
$\theta\in[\underline{\theta}_i,\overline{\theta}_i]$. If $\lambda_i^\Delta(\theta)$ shifts preferences toward higher (resp. lower) types proportionally more than $\lambda_i(\theta)$, in the sense that $\frac{\lambda_i^\Delta(\theta)}{\lambda_i(\theta)}$ is increasing on $[\underline{\theta}_i,\overline{\theta}_i]$
...
Lemma~\ref{lem:preferenceorder} thus connects the direction of a preference shift, toward higher or lower types, to the sign of $\Delta\G_i(\theta)$: tilting welfare weights proportionally toward higher types, while not decreasing the average weight, raises social weights pointwise, and conversely. Note that assuming a monotonic mean preserving perturbation ($\Delta \tilde \lambda_i =0$ and $\Delta \lambda_i'(\theta$) has a constant sign) is a constitutes a special case of the Lemma: if the rotation of $\lambda_i(\theta)$ is towards higher (resp. lower) weights then $\Delta\G_i(\theta)\;\ge\; (\text{resp.} \le)  \; 0$ for all $\theta\in[\underline{\theta}_i,\overline{\theta}_i]$.
...
\begin{proposition}[Elasticity condition]\label{prop:rho-q-cov-iron2}
Assume that both $R_\theta(\theta)$ and $R_q(\theta)$ are monotone and that $\Delta \G_i(\theta)$ has a constant sign for all $\theta$. Suppose that the following condition hold or all $\theta$:
\[\left|\frac{R_\Gamma'(\theta)}{R_\Gamma(\theta)}\right| \le  \left|\frac{R_\theta'(\theta)}{R_\theta(\theta)}\right|.\]

Then, for any sign configuration of $(R_\Gamma(\theta), R_\theta'(\theta), R_q'(\theta))$ such that $\mathrm{sign}(-R_\Gamma(\theta)R_\theta'(\theta))$ aligns with the sign of $\Delta R_\theta'(\theta)$, \[\mathrm{sign}\underbrace{\left(\Cov_{\theta_i}\!\left(\Delta R_\theta(\theta),R_q(\theta)\right)\right)}_\text{trade-off sorting effect} = \mathrm{sign}\!\left(-R_\Gamma(\theta) R_\theta'(\theta)\right) \cdot \mathrm{sign}\!\left(R_q'(\theta)\right).\] In sign configurations where $\mathrm{sign}(-R_\Gamma(\theta)R_\theta'(\theta))$ does not align with the sign of $\Delta R_\theta'(\theta)$, the elasticity condition is necessary but not sufficient to sign the covariance term.
\end{proposition}
...
Suppose $u$ exhibits constant absolute risk aversion with coefficient $\alpha>0$ and $\max_{k}\mathbb{E}\underline{CS}_i^\times(k_i)>0$. Then $R_q(\theta)=1/\alpha$ is constant, the behavioral channel vanishes, and the entire implementation region $[\tilde{k}_i^-,\tilde{k}_i^+]$ and its peak $k^\ast$ are governed by a single covariance condition:
\begin{enumerate}[label=(\roman*), leftmargin=1cm]
...
\node[font=\small] at (5.0,  -1.5)  {IR-unconstrained};
\node[font=\small] at (8.75, -1.5)  {IR-constrained};
\node[font=\small\bfseries, green!60!black]   at (1.25, 0.7)  {lower $\theta$ gain};
\node[font=\small\bfseries, orange!80!black]  at (3.75, 0.7)  {all $\theta$ gain};
\node[font=\small\bfseries, orange!60!black]  at (6.25, 0.7)  {higher $\theta$ gain};
\node[font=\small\bfseries, magenta!80!black] at (8.75, 0.7)  {higher $\theta$ gain};
\node[font=\footnotesize, blue!70!black] at (5.0, 1.7) {$\operatorname{Cov}(R_\Gamma,R_\theta)>0$: region expands (Cor.~\ref{cor:CARA})};
% arrows next to each threshold
\draw[->, thick, blue!70!black] (2.2, -0.55) -- (1.7, -0.55);
\draw[->, thick, blue!70!black] (4.7, -0.55) -- (4.2, -0.55);
\draw[->, thick, blue!70!black] (7.8, -0.55) -- (8.3, -0.55);
\draw[->, thick] (-0.2, 0) -- (10.5, 0);
\node[right] at (10.5, 0) {$k_i$};
\foreach \x/\lbl in {0/$0$, 2.5/$\tilde k_i^-$, 5/$k^\ast$, 7.5/$\tilde k_i^+$, 10/$k_i^+$}{
  \draw[thick] (\x, 0.2) -- (\x, -0.2);
...
\node[font=\small\bfseries, orange!60!black]  at (6.25, 0.7)  {higher $\theta$ gain};
\node[font=\small\bfseries, magenta!80!black] at (8.75, 0.7)  {higher $\theta$ gain};
\node[font=\footnotesize, red!70!black] at (5.0, 1.7) {$\operatorname{Cov}(R_\Gamma,R_\theta)<0$: region contracts (Cor.~\ref{cor:CARA})};
% arrows next to each threshold
\draw[->, thick, red!70!black] (2.8, -0.55) -- (3.3, -0.55);
\draw[->, thick, red!70!black] (5.3, -0.55) -- (5.8, -0.55);
\draw[->, thick, red!70!black] (7.2, -0.55) -- (6.7, -0.55);
\draw[->, thick] (-0.2, 0) -- (10.5, 0);
\node[right] at (10.5, 0) {$k_i$};
\foreach \x/\lbl in {0/$0$, 2.5/$\tilde k_i^-$, 5/$k^\ast$, 7.5/$\tilde k_i^+$, 10/$k_i^+$}{
  \draw[thick] (\x, 0.2) -- (\x, -0.2);
...
Proposition \ref{propch3:prop5} shows that it is possible to cover both fixed costs and participation constraints without distorting the allocation only when the level of investment is low. The intuition for this result can be understood as follows. First, we denote the marginal virtual utility: $J_i(q,\theta,s) = u(q^m_{i,k}(\theta,s),\theta,s) - \G_i(\theta)$, which is the marginal utility derived from the optimal allocation net of the information rent. Under the framework, it can be interpreted as the feasible gain in utility from the allocation after having remunerated the consumers to behave truthfully. Then, we can express the derivative of the $R-IR$ constraint for the first set of optimal quantities.
...
% \begin{equation*}
%     \G_i(\theta^1) - \G_i(\theta^2)  = \theta^2 - \theta^1
% \end{equation*}
...
\section{Proof of Proposition \ref{prop:FBalloc}} \label{ch3app:Ap1}
...
\begin{align*}
    \pdv{\mathcal{L}^m}{q_i} & = \int_{\mathcal S} \theta \; U(q_i(\theta,s),s) dF(s) - \varepsilon     +  \zeta \int_{\mathcal S}  (\theta \; U(q_i(\theta,s),s)  -\G_i(\theta)) dF(s)  
\end{align*}

Which gives the first-order condition:

\begin{equation}\label{eq:FOCIC}
    \theta \; U(q_i(\theta,s),s)  - \varepsilon     +  \zeta (\theta \; U(q_i(\theta,s),s)  -\G_i(\theta) )=0
\end{equation}
...
Finally, we show that the incentive compatibility condition is satisfied with the first-order condition. We derive the condition \ref{eq:FOCIC} with respect to the type $\theta $: $ \pdv{u}{q} \pdv{q_i}{\theta} + \pdv{u}{\theta}  + \varepsilon     +  \zeta (\pdv{u}{q} \pdv{q_i}{\theta} + \pdv{u}{\theta} -\pdv{\G_i}{\theta} )=0$. With the linear framework and uniform distribution  we have: $\pdv{u}{q} = -1$, $\pdv{u}{\theta} = 1$ and $\pdv{\G_i}{\theta} = -1 $. It implies that $ - \pdv{q_i}{\theta} + 1  + \varepsilon     +  \zeta (-\pdv{q_i}{\theta} + 2)=0$. That is:
\begin{equation*}
...
\begin{align}\label{eq:RIR}
        R^{IR}(k) = \sum_{i} \mu_i \int_{\Theta_i} \left[ \int_{0}^{s_1}  (U(q^m_{i,1}(\theta,s),\theta,s)  -\G_i(\theta)q^m_{i,1}(\theta,s) ) dF(s) + \right. \\
        \nonumber \left. \int_{s_1}^{\overline{s}}  (U(q^m_{i,2}(\theta,s),\theta,s)  -\G_i(\theta)q^m_{i,2}(\theta,s) ) dF(s) \right]dG_i(\theta) - I(k) 
\end{align}
...
\begin{align*}
        R^{IR}(k) = \sum_{i} \mu_i \int_{\Theta_i}  \int_{\mathcal S}  (U(q^m_{i,2}(\theta,s),\theta,s)  -\G_i(\theta)q^m_{i,2}(\theta,s) ) dF(s) dG_i(\theta) - I(k) 
\end{align*}

With the linear and uniform assumption, we have $q^m_{i,2}(\theta,s)= k + s + \theta - \sum_{i} \mu_i(s + \theta_i^{av})$ and $\G_i(\theta) = \overline{\theta}_i - \theta$. In that case: 
...
\begin{align*}
        \pdv{R^{IR}}{k} = & \sum_{i} \mu_i \int_{\Theta_i}  \int_{\mathcal S}  (u(q^m_{i,2}(\theta,s),\theta,s)\pdv{q^m_{i,2}}{k}  -\G_i(\theta)\pdv{q^m_{i,2}}{k}(\theta,s) )dF(s) dG_i(\theta)- r \\
        & \sum_{i} \mu_i \int_{\Theta_i}  \int_{\mathcal S}  (u(q^m_{i,2}(\theta,s),\theta,s) -\G_i(\theta)(\theta,s) )dF(s) dG_i(\theta)- r 
\end{align*}
...
\begin{equation*}
    u(q^m_{i,l},\theta,s)(1+\zeta) - \zeta \G_i(\theta) - \varepsilon =0
\end{equation*}
...
With $J_i(q^m_{i,l},\theta,s) = u(q^m_{i,l},\theta,s) - \G_i(\theta)$. It corresponds to the marginal virtual utility of a consumer. We start with the optimal off-peak allocation: $q_{i,3}$. In that case, given a realization of $s$, the capacity does not bind, and $\varepsilon = 0$. Hence
...
\begin{align*}
    & \sum_{i} \mu_i \int_{\Theta_i} \left[ \int_{0}^{s^m_1(k)}  (U(q^m_{i,3}(\zeta),\theta,s)  -\G_i(\theta)q^m_{i,3}(\zeta) ) dF(s)  \right. \\
      & +   \nonumber \left. \int_{s^m_1(k)}^{\overline{s}}  (U(q^m_{i,4}(\zeta,\varepsilon),\theta,s)  -\G_i(\theta)q^m_{i,4}(\zeta,\varepsilon) ) dF(s) \right]dG_i(\theta) - I(k) = 0
\end{align*}
...
\begin{align*}
        \sum_{i} \mu_i \int_{\Theta_i}  \int_0^{s_1}  (U(q^m_{i,1},\theta,s) - U(q^m_{i,2},\theta,s)  -\G_i(\theta)(q^m_{i,1} - q^m_{i,2}) ) dF(s) dG_i(\theta)
\end{align*}

As only the on-peak allocation $q^m_{i,2}$ depends on $k_i$, the first derivative with respect to $k_i$ gives:

\begin{align*}
        \sum_{i} \mu_i \int_{\Theta_i}  \int_0^{s_1}  (- u(q^m_{i,2},\theta,s)  + \G_i(\theta)) dF(s) dG_i(\theta)
\end{align*}
