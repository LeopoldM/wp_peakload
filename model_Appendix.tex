...
\[
(\mu_i\nu_i+\rho_i)\,u_q(q_i)\,\frac{\partial q_i(\theta,s)}{\partial k} \;+\; (\partial_k\rho_i)\,\theta u(q_i,s) \;-\; \mu_i\,\frac{\partial \varepsilon_i}{\partial k} \;=\; 0,
\]
hence
\begin{equation}\label{dqdk-general}
\frac{\partial q_i(\theta,s)}{\partial k}
=
\frac{\mu_i\,\frac{\partial \varepsilon_i}{\partial k}\;-\;(\partial_k\rho_i)\,\theta u(q_i,s)}
...
\partial_k q_i \;=\; \frac{\frac{\partial \varepsilon_i}{\partial k}-\partial_k\beta\,\theta u(q_i,s)}{\beta\,\theta u_q(q_i,s)}.
\end{equation}
...
\mathbb{E}_{w}[u|s]\;&:=\;
\frac{1}{\overline{w}^b(s)}\sum_{i\in N^{b}}\mu_i\mathbb{E}_{\theta_i}[w^{b}_i(\theta,s)\theta u(q_i,s)\,],
\\
\label{Eu2-def}\tag{$\mathbb{E}u^2$}
\mathbb{E}_{w}[u^2|s]  \;&:=\;
\frac{1}{\overline{w}^b(s)}\sum_{i\in N^{b}}\mu_i\mathbb{E}_{\theta_i}[w^{b}_i(\theta,s)(\theta u(q_i,s))^2\,].
\end{align}
...
\[
\sum_{i\in N^b}\mu_i\mathbb{E}_{(s,\theta_i)}[ \theta u(q_i,s)\,\frac{\partial q_i(\theta,s)}{\partial k}\,\mathbf 1_{T^*}(s)]\, \;=\; I'(k).
\]
...
Suppose the optimal investment $k^*>0$ is interior. Let $\mu^c := \sum_i \mu_i \Pr_i(\lambda_i(\theta)\leq\lambda^*)$ denote the total mass of contributing types and $\tilde\mu := \mu^c + \beta\sum_i \mu_i\,\E_{\theta_i}\!\left[\mathbf{1}_{\lambda_i(\theta)>\lambda^*}/\lambda_i(\theta)\right]$. Then
\begin{equation}\label{eq:FB-k1}
I'(k^*) = \frac{1}{\mu^c}\,\E_s\!\left[\sum_i \mu_i\, \E_{\theta_i}\!\left[\theta\, u\!\left(q_i^*(\theta,s),s\right)\mathbf{1}_{\lambda_i(\theta)\leq\lambda^*}\right]\mathbf{1}_{T^*}(s)\right],
\end{equation}
or equivalently, with $\tilde\mu < 1$,
\begin{equation}\label{eq:FB-k2}
I'(k^*) = \frac{1}{\tilde\mu}\,\E_s\!\left[\sum_i \mu_i\,\E_{\theta_i}\!\left[\theta\, u\!\left(q_i^*(\theta,s),s\right)\right]\mathbf{1}_{T^*}(s)\right].
\end{equation}
In the utilitarian case $\lambda_i(\theta)=\lambda$ for all $(i,\theta)$, all types contribute: $\tilde\mu=1$.
\end{lemma}
...
\textit{Step 2: non-contributing types.} From \eqref{eq:FB-T}, for every type with $\lambda_i(\theta)>\lambda^*$ and every $s\in T^*$:
\[
\theta\, u(q_i^*(\theta,s),s) = \frac{\varepsilon(s)}{\lambda_i(\theta)}.
\]
Multiplying by $\mu_i$, integrating over non-contributing types, and taking the expectation over $s\in T^*$:
\begin{equation}\label{eq:step2}
\E_s\!\left[\sum_i \mu_i\,\E_{\theta_i}\!\left[\theta\, u(q_i^*,s)\,\mathbf{1}_{\lambda_i(\theta)>\lambda^*}\right]\mathbf{1}_{T^*}(s)\right] = \E_s[\varepsilon(s)\mathbf{1}_{T^*}(s)]\sum_i \mu_i\,\E_{\theta_i}\!\left[\frac{\mathbf{1}_{\lambda_i(\theta)>\lambda^*}}{\lambda_i(\theta)}\right] = \beta\, I'(k)\sum_i \mu_i\,\E_{\theta_i}\!\left[\frac{\mathbf{1}_{\lambda_i(\theta)>\lambda^*}}{\lambda_i(\theta)}\right].
\end{equation}

\textit{Step 3: combine.} Adding \eqref{eq:step1} and \eqref{eq:step2}:
\[
\E_s\!\left[\sum_i \mu_i\,\E_{\theta_i}\!\left[\theta\, u(q_i^*,s)\right]\mathbf{1}_{T^*}(s)\right] = I'(k)\,\tilde\mu.
\]
Dividing by $\tilde\mu$ gives \eqref{eq:FB-k2}. Finally, since $\lambda_i(\theta)>\lambda^*=\beta$ for every non-contributing type, $\beta/\lambda_i(\theta)<1$, so
\[
\tilde\mu < \mu^c + \sum_i \mu_i\,\Pr_i(\lambda_i(\theta)>\lambda^*) = 1. \qedhere
\]
\end{proof}
...
\qquad
\omega_i^\times(\theta,s):=\frac{w_i^\times(\theta,s)}{\overline w_i^\times(s)},
\]
and the associated weighted expectation $\E_w[X\mid s]:=\E_{\theta_i}[\omega_i^\times(\theta,s)\,X(\theta,s)]$.
...
\[\partial_\theta \Delta q_i^\times(\theta,s) = \partial_\theta R_q(\theta,s)\cdot\left(R_{\mathcal{G}}(\theta)-\mathbb{E}_w[R_{\mathcal{G}}|s]\right) + R_q(\theta,s)\cdot R_{\mathcal{G}}'(\theta).\]

\[\partial_\theta \Delta q_i^\times(\overline\theta_i,s) = \partial_\theta R_q(\overline\theta_i,s)\cdot\left(\frac{\Delta\mu_i}{\mu_i}-\mathbb{E}_w[R_{\mathcal{G}}|s]\right)+R_q(\overline\theta_i,s)\cdot R_{\mathcal{G}}'(\overline\theta_i).\]


:
\[\partial_\theta \Delta q_i^\times(\underline\theta_i,s) = \partial_\theta R_q(\underline\theta_i,s)\cdot\left(R_{\mathcal{G}}(\underline\theta_i)-\mathbb{E}_w[R_{\mathcal{G}}|s]\right)+R_q(\underline\theta_i,s)\cdot R_{\mathcal{G}}'(\underline\theta_i).\]


\textbf{Differentiating with respect to $\theta$.}

Since $\mathbb{E}_w[R_{\mathcal{G}}|s]$ is constant in $\theta$, the product rule gives:
\[\partial_\theta \Delta q_i^\times(\theta,s) = \partial_\theta R_q(\theta,s)\cdot\left(R_{\mathcal{G}}(\theta)-\mathbb{E}_w[R_{\mathcal{G}}|s]\right) + R_q(\theta,s)\cdot R_{\mathcal{G}}'(\theta).\]
...
\[\partial_\theta \Delta q_i^\times(\overline\theta_i,s) = \partial_\theta R_q(\overline\theta_i,s)\cdot\left(\frac{\Delta\tilde\lambda_i}{\tilde\lambda_i}-\mathbb{E}_w[R_{\mathcal{G}}|s]\right)+R_q(\overline\theta_i,s)\cdot R_{\mathcal{G}}'(\overline\theta_i),\]
\[\partial_\theta \Delta q_i^\times(\underline\theta_i,s) = \partial_\theta R_q(\underline\theta_i,s)\cdot\left(\frac{\Delta\tilde\lambda_i}{\tilde\lambda_i}-\mathbb{E}_w[R_{\mathcal{G}}|s]\right)+R_q(\underline\theta_i,s)\cdot R_{\mathcal{G}}'(\underline\theta_i).\]

Since $\partial_\theta R_q(\theta,s)>0$ and $R_q(\theta,s)>0$ for all $\theta$, the sign of each endpoint derivative is determined entirely by the signs of $\Delta\tilde\lambda_i/\tilde\lambda_i - \mathbb{E}_w[R_{\mathcal{G}}|s]$ and $R_{\mathcal{G}}'$ respectively. The first terms are strictly positive multiples of the same scalar $\Delta\tilde\lambda_i/\tilde\lambda_i - \mathbb{E}_w[R_{\mathcal{G}}|s]$, hence share the same sign at both endpoints. Any difference in sign between the two endpoint derivatives can therefore only arise from the second terms $R_q\cdot R_{\mathcal{G}}'$, whose signs are determined by $R_{\mathcal{G}}'(\overline\theta_i)$ and $R_{\mathcal{G}}'(\underline\theta_i)$ respectively. Since these remain unrestricted, the derivative approach alone is insufficient to establish non-monotonicity, and the argument is better made directly on $\Delta q_i^\times$.
